...
async function runTests() {
  ...
  try {
    ...
    const character = await supabaseService.createCharacter(
      testUserId,
      characterData,
    );
    ...
    console.log('\n--- Test 2: Retrieve the character ---');
    const retrievedCharacter = await supabaseService.getCharacterById(
      characterId,
    );
    console.log(
      'Retrieved character:',
      retrievedCharacter ? 'Success' : 'Failed',
    );
    console.log(
      'Character data matches:',
      retrievedCharacter &&
        retrievedCharacter.name === characterData.name &&
        retrievedCharacter.original_description ===
          characterData.original_description
        ? 'Yes'
        : 'No',
    );
