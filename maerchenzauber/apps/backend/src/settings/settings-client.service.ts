...
export class SettingsClientService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.serviceKey =
      this.configService.get<string>('MANA_SUPABASE_SECRET_KEY') || '';

    if (!this.manaServiceUrl) {
      this.logger.warn(
        'MANA_SERVICE_URL not configured - user settings integration disabled',
      );
    }
  }
  ...
  async getUserSettings(token: string): Promise<UserSettings> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        this.logger.error(
          `Failed to get user settings: ${response.status} - ${errorText}`,
        );
        return {};
      }
  ...
  async getUserSettingsAsService(userId: string): Promise<UserSettings> {
    ...
    try {
      const response = await fetch(
        `${this.manaServiceUrl}/users/settings/service/${userId}`,
        {
          headers: {
  ...
  async getMaerchenzauberSettings(
    userToken: string,
  ): Promise<MaerchenzauberSettings> {
    try {
  ...
  async updateServiceSettings(
    settings: Partial<MaerchenzauberSettings>,
    token: string,
  ) {
    if (!this.manaServiceUrl) {
      return settings;
    ...
    try {
      const response = await fetch(
        `${this.manaServiceUrl}/users/settings/maerchenzauber`,
        {
          method: 'PATCH',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  async getUserPreferredAuthorId(
    userToken: string,
  ...
  async getUserPreferredIllustratorId(
    userToken: string,
  ): Promise<string | undefined> {
    const settings = await this.getMaerchenzauberSettings(userToken);
