...
export class CharacterController {
  ...
  async generateCharacterImages(
    ...
    this.logger.log(
      `Generating character images for user ${user.email} (${user.sub})`,
    );
    ...
    try {
      ...
      const characterDescriptionPrompt_2 =
        await this.promptingService.createCharacterDescriptionPrompt(
          promptedDescription,
          0.9,
        );
      ...
      if (
        characterDescriptionPrompt_2.error ||
        characterDescriptionPrompt_3.error
      ) {
        return {
      ...
      if (image_original.error || image_2.error || image_3.error) {
        ...
        if (image_original.error)
          this.logger.error('Image 1 error:', image_original.error);
        ...
        const firstError =
          image_original.error || image_2.error || image_3.error;
        throw firstError;
      }
      ...
      const blurHash = this.imageService.getBlurHashForUrl(
        image_original.data!,
      );
  ...
  async generateAnimalCharacter(
    ...
    this.logger.log(
      `Generating animal character for user ${user.email} (${user.sub})`,
    );
    ...
    try {
      ...
      const enhancedResult =
        await this.promptingService.enhanceAnimalCharacterDescription(
          userDescription,
          name,
        );
      ...
      if (enhancedResult.error) {
        this.logger.warn(
          'Failed to enhance character description, using original',
        );
      }

      const characterDescription = enhancedResult.data || userDescription;
      this.logger.log(
        `Enhanced character description: ${characterDescription}`,
      );
      ...
      const animalType = await this.promptingService.detectAnimalType(
        userDescription,
      );
      ...
      const characterDescriptionPrompt_2 =
        await this.promptingService.createAnimalCharacterDescriptionPrompt(
          imagePrompt,
          0.9,
        );
      const characterDescriptionPrompt_3 =
  ...
  async getCharacters(
    ...
    this.logger.log(`Fetching characters for user ${user.email} (${user.sub}), includeArchived: ${includeArchived}`);
    ...
    try {
      ...
      this.logger.log(
        `Found ${characters.length} characters for user ${user.sub}`,
      );
      ...
      const transformedCharacters = characters.map((char) => ({
        ...
        characterDescriptionPrompt:
          char.character_description_prompt || char.character_description,
        images:
  ...
  async getCharacterById(
    ...
    this.logger.log(
      `Fetching character ${id} for user ${user.email} (${user.sub})`,
    );

    try {
      const character = await this.supabaseAuthService.getCharacterById(
        id,
        token,
      );
  ...
  async updateCharacter(
    ...
    this.logger.log(
      `Updating character ${id} for user ${user.email} (${user.sub})`,
    );
  ...
  async deleteCharacter(
    ...
    this.logger.log(
      `Deleting character ${id} for user ${user.email} (${user.sub})`,
    );
  ...
  async publishCharacter(
    ...
    this.logger.log(
      `Publishing character ${body.characterId} for user ${user.sub}`,
    );
  ...
  async unpublishCharacter(
    ...
    this.logger.log(
      `Unpublishing character ${characterId} for user ${user.sub}`,
    );
  ...
  async getSharedCharacter(
    ...
    this.logger.log(
      `User ${user.sub} fetching shared character ${characterId}`,
    );

    try {
      // Use service account to fetch character (bypasses RLS)
      const character =
        await this.supabaseAuthService.getSharedCharacter(characterId);
  ...
  async importCharacter(
    ...
    this.logger.log(
      `User ${user.email} (${user.sub}) importing character ${characterId}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `Successfully consumed 10 credits for character import by user ${user.sub}`,
      );
      ...
      const transformedCharacter = {
        ...
        originalDescription:
          importedCharacter.user_description ||
          importedCharacter.original_description,
        characterDescriptionPrompt:
          importedCharacter.character_description_prompt ||
          importedCharacter.character_description,
        images:
  ...
  async generateAnimalFromImage(
    ...
    try {
      ...
      const analysisResult = await this.imageService.analyzeImage(
        uploadedImageUrl!,
      );
      ...
      if (analysisResult.error) {
        ...
        userDescription =
          'A domestic animal with fur, four legs, and a friendly expression.';
        console.log('Using fallback description:', userDescription);
      } else {
        userDescription = analysisResult.data || '';
        console.log(
          'Successfully analyzed image, description:',
          userDescription,
        );
      }
      ...
      if (enhancedResult.error) {
        this.logger.warn(
          'Failed to enhance character description from image, using original',
        );
      }
