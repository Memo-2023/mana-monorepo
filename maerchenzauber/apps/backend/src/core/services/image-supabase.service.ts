...
export class ImageSupabaseService {
  ...
  async generateImageWithReplicate(
    ...
    try {
      ...
      const output = Array.isArray(replicateOutput)
        ? replicateOutput[0]
        : replicateOutput;
      ...
      this.logger.log(
        `Successfully generated image with Replicate: ${uploadResult.url}`,
      );
      ...
      return createFailure(
        new Error(`Replicate generation failed: ${errorMessage}`),
        {
  ...
  private async uploadToSupabaseStorage(
    ...
    const optimizedImages = await this.imageOptimizationService.optimizeImage(
      imageBuffer,
    );

    // Generate BlurHash for smooth placeholders
    const blurHash = await this.imageOptimizationService.generateBlurHash(
      imageBuffer,
    );
    ...
    if (token) {
      ...
      this.logger.log(
        'No JWT token provided for storage upload - using anon key only',
      );
    }
    ...
    if (errors.length > 0) {
      ...
      throw new Error(
        `Failed to upload to Supabase Storage: ${errors[0].error?.message}`,
      );
    }
   ...
   */
  async createSignedUrlForCharacterImage(
    imageUrl: string,
  ): Promise<Result<string>> {
    try {
      ...
      if (imageUrl.includes('/storage/v1/object/public/')) {
        this.logger.log(
          `Image URL is already public, using directly: ${imageUrl.substring(
            0,
            80,
          )}...`,
        );
  ...
  async saveUploadedImage(
    ...
    try {
      const result = await this.uploadToSupabaseStorage(
        imageBuffer,
        path,
        fileExtension,
        token,
    ...
    switch (modelId) {
      ...
      case 'seedream-4':
        ...
        if (characterImageUrl && isStoryImage) {
          ...
          this.logger.log(
            `Using character image for consistent generation: ${characterImageUrl}`,
          );
        }
      ...
      case 'sd-3.5-large-turbo':
        return {
          prompt: `${prompt}. ${defaultEnhancement}`,
          negative_prompt:
            'scary, dark, violent, horror, gore, adult content, inappropriate',
  ...
  async generateBatchImages(
    ...
    try {
      ...
      const batchResult = await this.replicateClient.run(
      ...
      return {
        data: null,
        error:
          error instanceof Error ? error : new Error('Batch generation failed'),
      };
    }
  }
}
