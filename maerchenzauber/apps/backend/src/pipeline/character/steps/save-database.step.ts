...
export const saveToDatabase: PipelineStep<
  CharacterImageOutput,
  CharacterDatabaseOutput
> = {
  ...
  execute: async (input, context) => {
    ...
    try {
      ...
      throw new Error(
        `Failed to save character to database: ${(error as Error).message}`,
      );
    }
  },
  ...
  rollback: async (input, error, context) => {
    if ('characterId' in input && input.characterId) {
      console.log(
        `[Rollback] save-to-database: Deleting character ${input.characterId}`,
      );
