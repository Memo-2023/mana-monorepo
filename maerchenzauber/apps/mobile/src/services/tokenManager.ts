...
class TokenManager {
  ...
  private notifyObservers(state: TokenState, token?: string): void {
    this.observers.forEach(observer => {
      try {
        ...
        debug('Error in token state observer:', error);
      }
    });
  }
  ...
  async getValidToken(): Promise<string | null> {
    try {
      ...
      // Safety catch for any unexpected errors during token retrieval
      debug('TokenManager: Unexpected error in getValidToken:', error);
      return null;
    }
  }
  ...
  private async queueRequest(
    ...
    return new Promise((resolve, reject) => {
      ...
      debug(`TokenManager: Queued request ${queueItem.id}, queue size: ${this.requestQueue.length}`);
    });
  }
    ...
    for (let attempt = 0; attempt < retryDelays.length; attempt++) {
      try {
        ...
        if (retryDelays[attempt] > 0) {
          debug(`TokenManager: Retrying token refresh in ${retryDelays[attempt]}ms (attempt ${attempt + 1}/${retryDelays.length})`);
