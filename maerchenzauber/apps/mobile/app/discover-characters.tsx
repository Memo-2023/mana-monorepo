...
import { View, StyleSheet, FlatList, Dimensions, TouchableOpacity, ActivityIndicator, RefreshControl } from 'react-native';
...
export default function DiscoverCharactersScreen() {
  ...
  const handleCharacterPress = useCallback((character: any) => {
    // Navigate to character detail page
    router.push(`/character/${character.id}`);
  }, [router]);

  const handleVote = useCallback(async (characterId: string, voteType: 'like' | 'love' | 'star') => {
    await voteForCharacter(characterId, voteType);
  }, [voteForCharacter]);

  const handleClone = useCallback(async (characterId: string) => {
      ...
      return (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyStateText}>
            ...
            {"\n\n"}Du kannst bereits deine eigenen Charaktere über den Share-Button veröffentlichen.
          </Text>
        </View>
      );
    }
