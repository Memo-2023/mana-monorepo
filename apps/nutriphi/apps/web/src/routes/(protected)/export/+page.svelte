<script lang="ts">
  ...
  async function handleExport() {
    ...
    try {
      ...
      const blob = new Blob(['Export data'], { type: format === 'csv' ? 'text/csv' : 'application/pdf' });
...
<div class="mx-auto max-w-2xl space-y-6">
      ...
      <div class="space-y-3">
        <label class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400">Alle erfassten Mahlzeiten mit Nährwerten</p>
          </div>
        </label>
        <label class="flex items-center gap-3">
          <input
          ...
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400">Tägliche Zusammenfassungen und Trends</p>
          </div>
        </label>
        <label class="flex items-center gap-3">
          <input
          ...
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400">Deine Ernährungsziele und deren Erreichung</p>
          </div>
        </label>
      </div>
    </div>
  ...
  <div class="rounded-2xl bg-blue-50 p-4 dark:bg-blue-900/20">
    <p class="text-sm text-blue-700 dark:text-blue-400">
      <strong>Hinweis:</strong> Der Export enthält alle Daten im gewählten Zeitraum.
      CSV-Dateien können in Excel, Google Sheets oder ähnlichen Programmen geöffnet werden.
    </p>
  </div>
</div>
