<script lang="ts">
	...
	const isFullHeightPage = $derived(
		$page.url.pathname === '/record' ||
			$page.url.pathname === '/memos' ||
	...
	const languageItems: PillDropdownItem[] = $derived(
		languages.map((lang) => ({
			id: lang.code,
			label: lang.label,
			onClick: () => {
...
{:else}
	...
	<div class="flex flex-col min-h-screen">
		...
		<main
			class="flex-1 main-content transition-all duration-300 {isCollapsed
				? ''
				: isSidebarMode
					? 'pl-[180px]'
