<script lang="ts">
	...
	// Check if file is a valid media file (audio or video)
	...
	function handleFileSelect(event: Event) {
		...
		if (input.files && input.files[0]) {
			...
			if (isValidMediaFile(file)) {
				...
				uploadError = 'Bitte wähle eine Audio- oder Videodatei aus';
			}
		}
	}
	...
	function handleDrop(event: DragEvent) {
		...
		if (files && files[0]) {
			...
			if (isValidMediaFile(file)) {
				...
				uploadError = 'Bitte lege eine Audio- oder Videodatei ab';
			}
		}
	}
	...
	async function handleUpload() {
		...
		try {
			...
			const duration = await getMediaDuration(selectedFile, mediaType);
...
<svelte:head>
	<title>Medien hochladen - Memoro</title>
</svelte:head>
...
<div class="flex h-full flex-col">
	<div class="flex-1 overflow-y-auto">
		<div class="mx-auto max-w-5xl px-4 py-4">
			<!-- Header -->
			<div class="mb-4">
				<Text variant="large" weight="bold" class="mb-1 text-2xl">Medien hochladen</Text>
				<Text variant="small" class="text-theme-secondary">
					Lade eine Audio- oder Videodatei hoch und verarbeite sie mit AI
				</Text>
			</div>
			...
			{:else}
				...
				<div class="mb-4">
					<div class="rounded-xl border border-theme bg-menu p-4">
						<Text variant="large" weight="semibold" class="mb-3"
							>Audio- oder Videodatei hochladen</Text
						>
						...
						>
							{#if selectedFile}
								<!-- Selected File -->
								<div
									class="flex items-center justify-between rounded-lg border border-theme bg-menu p-3"
								>
									<div class="flex items-center gap-3">
										{#if selectedFile.type.startsWith('video/')}
											<!-- Video Icon -->
											<svg
												class="h-6 w-6 text-mana"
												fill="none"
												stroke="currentColor"
												viewBox="0 0 24 24"
											>
										...
										<div class="text-left">
											...
											<Text variant="small" class="text-theme-secondary">
												{formatFileSize(selectedFile.size)} • {selectedFile.type.startsWith(
													'video/'
												)
													? 'Video'
													: 'Audio'}
							...
							{:else}
								...
								<Text variant="muted" class="mt-2 text-xs"
									>Audio: MP3, M4A, WAV, WEBM | Video: MP4, MOV, AVI, MKV, WEBM</Text
								>
							{/if}
						</div>
					</div>
				...
				<div class="mb-4 rounded-xl border border-theme bg-menu p-4">
					...
					<div class="space-y-4">
						<!-- Title -->
						<div>
							<label for="title" class="mb-1.5 block text-sm font-medium text-theme"
						...
						<!-- Blueprint -->
						<div>
							<label for="blueprint" class="mb-1.5 block text-sm font-medium text-theme"
							...
							>
								...
								{#each blueprints as blueprint}
									<option value={blueprint.id}
										>{blueprint.name?.de || blueprint.name?.en || blueprint.id}</option
									>
								{/each}
							</select>
						</div>
						...
						<div class="grid grid-cols-2 gap-3">
							...
							<!-- Time -->
							<div>
								<label for="time" class="mb-1.5 block text-sm font-medium text-theme"
							...
							<div class="flex flex-wrap gap-2">
								{#each availableLanguages as lang}
									<button
										...
										class="rounded-lg border-2 px-3 py-1.5 text-sm font-medium transition-all {recordingLanguages.includes(
											lang.code
										)
						...
						<div class="flex items-center justify-between">
							<div>
								<label for="diarization" class="text-sm font-medium text-theme"
									>Sprechererkennung</label
								>
							...
							<button
								...
								class="relative inline-flex h-7 w-12 items-center rounded-full transition-colors {enableDiarization
									? 'bg-mana'
									: 'bg-gray-300 dark:bg-gray-600'}"
							>
								<span
									class="inline-block h-5 w-5 transform rounded-full bg-white transition-transform {enableDiarization
										? 'translate-x-6'
										: 'translate-x-1'}"
								></span>
							</button>
				...
				<div class="flex justify-end gap-3">
					<button onclick={() => goto('/dashboard')} class="btn-secondary"> Abbrechen </button>
					<button
