				...
				<button
					...
					style="width: 160px; background-color: {hoveredApp === index
						...
						: isDark
							...
							: 'rgba(0, 0, 0, 0.05)'}; border: 1px solid {isDark
						? 'rgba(255, 255, 255, 0.1)'
						: 'rgba(0, 0, 0, 0.1)'}; box-shadow: 0 4px 20px rgba(0, 0, 0, {isDark
						? '0.3'
						: '0.15'}); transform: perspective(1000px) rotateX({cardRotations[index]?.rotateX ||
						0}deg) rotateY({cardRotations[index]?.rotateY ||
				...
				{#each apps as app, index}
					<div
						...
						style="min-width: 360px; max-width: 360px; background-color: {hoveredApp === index
							...
							0}deg); transform-style: preserve-3d; transition: transform 0.1s ease-out, background-color 0.2s ease-out; opacity: 1;"
						onclick={(e) => {
							e.stopPropagation();
							selectedApp = index;
						}}
