...
const features = defineCollection({
  ...
  schema: z.object({
    ...
    category: z.enum(['organization', 'language', 'customization', 'recording', 'analytics', 'collaboration', 'ai-features', 'sharing']).optional(),
    order: z.number().optional(),
  })
});
...
const calendarCollection = defineCollection({
  ...
  schema: z.object({
    ...
    events: z.array(z.object({
      ...
      contentType: z.enum(['blog', 'changelog', 'statistics', 'feature', 'guide', 'testimonial', 'faq', 'social', 'newsletter', 'event']),
      contentCollection: z.string().optional(), // z.B. "blog", "changelog"
      status: z.enum(['idea', 'planned', 'writing', 'review', 'scheduled', 'published', 'cancelled']).default('planned'),
...
const _storiesCollection = defineCollection({
  ...
  schema: z.object({
    ...
    // Meta-Informationen
    status: z.enum(['concept', 'draft', 'review', 'approved', 'produced', 'archived']).default('draft'),
    visibility: z.enum(['internal', 'team', 'stakeholders']).default('internal'),
    tags: z.array(z.string()).optional(),
