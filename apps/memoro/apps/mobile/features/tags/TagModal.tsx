...
const ModalTagCreateAndEdit = ({
    ...
    <View className='w-full'>
      ...
      <View className='w-full'>
        <Text className={`${isDark ? 'text-white/80' : 'text-black/80'}`}>
          {editingTag ? t('tags.change_color', 'Farbe ändern:') : t('tags.select_color', 'Farbe auswählen:')}
        </Text>
        <ColorPicker
          colors={TAG_COLORS}
  ...
  return (
    <BaseModal
      isVisible={isVisible}
      onClose={onCancel}
      title={editingTag ? t('tags.edit_tag', 'Tag bearbeiten') : t('tags.create_tag', 'Tag erstellen')}
      animationType='fade'
