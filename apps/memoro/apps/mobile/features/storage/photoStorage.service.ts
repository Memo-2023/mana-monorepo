...
class PhotoStorageService {
	...
	async selectPhotos(options?: {
		...
		try {
			// Android 13+ Photo Picker doesn't require permissions - users explicitly
			// select which photos to share. iOS still requires explicit permission.
			if (Platform.OS === 'ios') {
	 ...
	 */
	async processImage(uri: string, config: Partial<PhotoProcessingConfig> = {}): Promise<string> {
		try {
	 ...
	 */
	async uploadPhotos(memoId: string, photos: ImagePickerAsset[]): Promise<PhotoUploadResult[]> {
