...
const MEMORO_SERVICE_URL =
	process.env.EXPO_PUBLIC_MEMORO_MIDDLEWARE_URL?.replace(/\/$/, '') ||
...
export const triggerTranscription = async ({
	...
	try {
		...
		if (networkErrorInfo.isNetworkError) {
			...
			const errorMsg = t(
				'audio_archive.transcription_error',
				'Beim Starten der Transkription ist ein Fehler aufgetreten.'
			);
			if (showAlerts) {
				Alert.alert(t('common.error', 'Fehler'), errorMsg);
			}
