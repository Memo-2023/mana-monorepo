...
export const useUploadStatusStore = create<UploadStatusStoreState>((set, get) => ({
  ...
  updateStatus: async (
    ...
    const updatedMetadata: UploadMetadata = {
      ...
      uploadedAt:
        status === UploadStatus.SUCCESS ? Date.now() : currentMetadata.uploadedAt,
      // Update lastAttemptAt when uploading
      lastAttemptAt:
        status === UploadStatus.UPLOADING ? Date.now() : currentMetadata.lastAttemptAt,
    };
