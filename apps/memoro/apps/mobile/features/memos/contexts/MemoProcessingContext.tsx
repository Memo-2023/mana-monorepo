...
export const MemoProcessingProvider: React.FC<{children: React.ReactNode}> = ({ children }) => {
  ...
  const calculateDisplayTitle = useCallback((
    ...
    console.debug('🔍 calculateDisplayTitle inputs:', { title, transcriptionStatus, headlineStatus });
    
    // HIGHEST PRIORITY: If headline generation is completed AND we have a real title, use it
    if (headlineStatus === 'completed' && title && title !== 'Unbenanntes Memo' && title !== 'Neue Aufnahme' && title.trim() !== '') {
    ...
    if (headlineStatus === 'completed' && (!title || title === 'Unbenanntes Memo' || title === 'Neue Aufnahme' || title.trim() === '')) {
      console.debug('⚠️ Headline completed but no valid title, continuing to show generation status');
    ...
    if (transcriptionStatus === 'completed' && (!headlineStatus || headlineStatus === 'processing')) {
  ...
  const updateProcessingState = useCallback((newState: Partial<MemoProcessingState>) => {
    ...
    console.debug('📝 Updating processing state:', {
      ...
      transcriptionStatus: newState.transcriptionStatus !== undefined ? newState.transcriptionStatus : processingStateRef.current?.transcriptionStatus,
      headlineStatus: newState.headlineStatus !== undefined ? newState.headlineStatus : processingStateRef.current?.headlineStatus,
    });
    ...
    const newDisplayTitle = calculateDisplayTitle(
      ...
      newState.transcriptionStatus !== undefined ? newState.transcriptionStatus : processingStateRef.current?.transcriptionStatus || null,
      newState.headlineStatus !== undefined ? newState.headlineStatus : processingStateRef.current?.headlineStatus || null
    );
  ...
  useAllMemoUpdates((payload) => {
    if (payload.event === 'UPDATE') {
      console.debug('🔄 REALTIME UPDATE for memo:', payload.new.id);
      console.debug('📝 REALTIME UPDATE memo title:', payload.new.title);
      console.debug('📝 REALTIME UPDATE transcription status:', payload.new.metadata?.processing?.transcription?.status);
      console.debug('📝 REALTIME UPDATE headline status:', payload.new.metadata?.processing?.headline_and_intro?.status);
      
      // Check if this is the current memo
      const isCurrentMemo = currentMemoRef.current && currentMemoRef.current.id === payload.new.id;
  ...
  return (
    <MemoProcessingContext.Provider value={value}>
      {children}
    </MemoProcessingContext.Provider>
  );
};
