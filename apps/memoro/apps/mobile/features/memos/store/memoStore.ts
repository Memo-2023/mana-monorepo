...
interface MemoState {
  ...
  setUploadingPlaceholder: (memoId: string, audioFileId: string, title?: string, spaceId?: string | null) => void;
...
export const useMemoStore = create<MemoState>((set, get) => ({
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        if (latestMemoPath && memosByPath[latestMemoPath]?.length > 1) {
          ...
          console.debug(`Mehrere Einträge (${memosByPath[latestMemoPath].length}) für Pfad:`, latestMemoPath);
          
          // Suche nach einem Memo mit Titel (höchste Priorität)
          let completedMemo = memosByPath[latestMemoPath].find((memo) => 
          ...
          if (!completedMemo) {
            completedMemo = memosByPath[latestMemoPath].find((memo) => 
          ...
          if (!completedMemo) {
            completedMemo = memosByPath[latestMemoPath].find((memo) => 
              memo.metadata?.processing?.transcription?.status === 'completed'
            );
          }
        ...
        console.debug('Memo-Details:', {
          ...
          transcriptionStatus: bestMemo.metadata?.processing?.transcription?.status || bestMemo.metadata?.transcriptionStatus,
  ...
  incrementLocalViewCount: (id) => set((state) => {
  ...
  updateMemo: (memoId, updates) => set((state) => {
    const { latestMemo } = state;
    ...
    if (latestMemo && latestMemo.id === memoId) {
      ...
      console.log(`MemoStore: Updated memo ${memoId} in store - title changed from "${latestMemo.title}" to "${updatedMemo.title}"`);
