	...
	const loadMemoData = useCallback(
		async (memoId: string, retryCount = 0) => {
			...
			try {
				...
				// Generate audio URL if audio_path exists - LOCAL-FIRST approach
				// First check if we have a local copy of the audio file, then fall back to remote
				...
				if (data?.source?.audio_path) {
					...
					if (audioPath) {
						// Step 1: Try to find local audio file first (faster, works offline)
						debug('MemoState: Checking for local audio file...');
						const localAudioPath = await fileStorageService.getLocalAudioForMemo(memoId);

						if (localAudioPath) {
							// Use local file - no network request needed!
							setAudioUrl(localAudioPath);
							debug('MemoState: Using LOCAL audio file:', localAudioPath);
						} else {
							// Step 2: Fall back to remote signed URL
							debug(
								'MemoState: No local file found, requesting signed URL for audio path:',
								audioPath
							);
							const signedUrl = await getSignedUrl(audioPath);
							if (signedUrl) {
								setAudioUrl(signedUrl);
								debug('MemoState: Using REMOTE audio URL:', signedUrl.substring(0, 100) + '...');
							} else {
