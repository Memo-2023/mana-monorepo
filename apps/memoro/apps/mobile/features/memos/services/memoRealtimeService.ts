...
class MemoRealtimeService {
  ...
  private handleMemoChange = (event: 'INSERT' | 'UPDATE' | 'DELETE', payload: any) => {
    ...
    console.log(`MemoRealtimeService: ${event} event details:`, {
      ...
      changes: event === 'UPDATE' ? {
        ...
        metadataChanged: JSON.stringify(payload.old?.metadata) !== JSON.stringify(payload.new?.metadata),
        sourceChanged: JSON.stringify(payload.old?.source) !== JSON.stringify(payload.new?.source)
      } : null
    });
  ...
  private subscribe(eventKey: string, callback: MemoEventCallback): () => void {
    ...
    return () => {
      ...
      if (index > -1) {
        ...
        console.log(`MemoRealtimeService: Removed listener for ${eventKey}, remaining: ${callbacks.length}`);
      }
    };
  }
  ...
  private handleSystemEvent = (payload: any) => {
    ...
    if (payload.status === 'error' && payload.message) {
      ...
      if (errorMessage.includes('Unable to subscribe to changes') && 
          errorMessage.includes('Please check Realtime is enabled')) {
        console.error('MemoRealtimeService: Realtime not enabled for memos table. Please enable via Supabase dashboard or SQL: ALTER PUBLICATION supabase_realtime ADD TABLE public.memos;');
    ...
    switch (status) {
      ...
      case 'CHANNEL_ERROR':
        ...
        if (shouldLog && isOnline) {
          console.warn(`MemoRealtimeService: Channel error detected (${this.lastErrorStatus.count} occurrences)`);
        ...
        if (error && typeof error === 'string' && error.includes('Realtime not enabled')) {
          console.error('MemoRealtimeService: Realtime not enabled for memos table. Please enable in Supabase dashboard.');
          this.isInitialized = false; // Prevent further reconnection attempts
          return;
        }
  ...
  private async handleTokenExpiration(): Promise<void> {
    ...
    try {
      ...
      if (newToken) {
        ...
        console.warn('MemoRealtimeService: Could not get fresh token, connection may remain broken');
      }
    } catch (error) {
  ...
  private async shouldAttemptReconnect(_error?: any): Promise<boolean> {
    ...
    const { isDeviceConnected, hasStableConnection } = await import('~/features/errorHandling/utils/networkErrorUtils');
    const isOnline = await isDeviceConnected();
    ...
    if (!isStable) {
      console.debug('MemoRealtimeService: Connection not stable yet, will wait before reconnecting');
  ...
  private scheduleReconnect(): void {
    ...
    if (timeSinceLastAttempt < this.MIN_RECONNECT_INTERVAL_MS) {
      console.log(`MemoRealtimeService: Skipping reconnect - too soon since last attempt (${timeSinceLastAttempt}ms ago)`);
      return;
    }
    ...
    console.log(`MemoRealtimeService: Scheduling reconnect attempt ${this.reconnectAttempts + 1} in ${actualDelay}ms`);
  ...
  private async attemptReconnect(): Promise<void> {
    ...
    console.log(`MemoRealtimeService: Attempting reconnect ${this.reconnectAttempts}/${this.MAX_RECONNECT_ATTEMPTS}`);
  ...
  async subscribeToMemoWithInitialData(
    ...
    if (includeInitialFetch) {
      try {
        ...
        if (!this.supabaseClient) {
          console.log('MemoRealtimeService: No Supabase client available yet, attempting to get authenticated client');
          this.supabaseClient = await getAuthenticatedClient();
        }

        // Check again after attempting to get client
        if (!this.supabaseClient) {
          console.warn('MemoRealtimeService: Unable to get authenticated client for initial data fetch');
  ...
  subscribeToMemoTitle(
    ...
    const getDisplayTitle = (memo: any): string => {
      ...
      if (memo.metadata?.processing?.headline_and_intro?.status === 'completed') {
        ...
        const headlineFromMetadata = memo.metadata?.processing?.headline_and_intro?.details?.headline;
        if (headlineFromMetadata && headlineFromMetadata.trim() !== '') {
          console.log(`MemoRealtimeService: Using headline from metadata: "${headlineFromMetadata}"`);
      ...
      if (transcriptionStatus === 'completed' && (!headlineStatus || headlineStatus === 'pending')) {
        return t ? t('memo.status.headline_generating') : 'Generating Headline';
      }
    ...
    this.subscribeToMemoWithInitialData(
      ...
      (memo, isInitial) => {
        ...
        console.log(`MemoRealtimeService: Title for memo ${memoId}: "${title}" (initial: ${isInitial})`);
  ...
  subscribeToBroadcastChannel(
    ...
    if (!this.supabaseClient) {
      console.warn('MemoRealtimeService: No authenticated client available for broadcast subscription');
      return () => {};
    }
...
export const testRealtimeUpdate = (memoId: string) => memoRealtimeService.testRealtimeUpdate(memoId);
