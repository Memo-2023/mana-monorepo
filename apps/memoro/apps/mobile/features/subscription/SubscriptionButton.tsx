...
export const SubscriptionButton: React.FC<SubscriptionButtonProps> = ({
	...
	return (
		<Pressable
			className="h-12 px-4 rounded-lg items-center justify-center flex-row justify-between border"
			style={[
				{
		...
		>
			<View className="flex-row items-center justify-center">
				<Icon name={leftIconName} size={16} color={getTextColor()} style={{ marginRight: 8 }} />
				<Text variant="body" style={[{ color: getTextColor(), fontWeight: '500' }]}>
					{label}
				</Text>
			</View>

			<Icon name={iconName} size={16} color={getTextColor()} style={{ marginLeft: 8 }} />
		</Pressable>
	);
};
