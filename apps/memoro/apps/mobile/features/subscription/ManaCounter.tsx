 ...
 */
const ManaCounter: React.FC<ManaCounterProps> = ({ count }) => {
	...
	useEffect(() => {
		console.log(
			'[ManaCounter] useEffect - useLocalState:',
			useLocalState,
			'contextCredits:',
			contextCredits
		);
	...
	const containerClasses =
		'flex-row items-center justify-center border rounded-full min-w-[80px] self-start';
	const containerHeight = 52;
	const iconSize = 24;
	const textClasses = 'font-semibold text-xl';
	...
	return (
		<Pressable
			...
			style={{
				...
				opacity: Platform.OS === 'web' && isHovered ? 0.9 : 0.66,
			}}
			onPress={handlePress}
			onHoverIn={() => Platform.OS === 'web' && setIsHovered(true)}
			onHoverOut={() => Platform.OS === 'web' && setIsHovered(false)}
		>
			<ManaIcon size={iconSize} color="#0099FF" style={{ marginRight: 1, marginLeft: -6 }} />
			<Text className={tw(textClasses)} style={{ color: '#66CCFF', marginRight: 4 }}>
				Mana:
			</Text>
