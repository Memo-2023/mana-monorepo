		...
		const unsubscribe = NetInfo.addEventListener((state) => {
			...
			if (
				...
				reconnectionTimeout = setTimeout(async () => {
					...
					try {
						const { tokenManager, TokenState } = await import(
							'~/features/auth/services/tokenManager'
						);
					...
					try {
						const { default: memoRealtimeService } = await import(
							'~/features/memos/services/memoRealtimeService'
						);
		...
		<NetworkStatusContext.Provider value={value}>
			...
			{networkState && !networkState.isConnected && (
				<View style={[styles.offlineBanner, { backgroundColor: isDark ? '#ff6b6b' : '#e74c3c' }]}>
					<Text style={styles.offlineText}>{t('common.offline', 'No internet connection')}</Text>
				</View>
			)}
