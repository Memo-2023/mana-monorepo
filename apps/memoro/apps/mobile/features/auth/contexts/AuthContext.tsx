...
import { runLocalAudioCleanupIfNeeded } from '~/features/storage/services/localAudioCleanup';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	...
	useEffect(() => {
		...
		const initAuth = async () => {
			try {
				...
				if (authenticated) {
					...
					if (userData) {
						...
						try {
							...
							console.debug('Error setting Supabase token during startup (non-fatal):', error);
						}
						...
						try {
							...
							console.error(
								'Error with RevenueCat initialization for existing user (non-fatal):',
								error
							);
						}

						// Run local audio cleanup in background (if auto-delete setting is enabled)
						try {
							runLocalAudioCleanupIfNeeded().catch(console.debug);
						} catch (cleanupError) {
							console.debug('Audio cleanup error (non-fatal):', cleanupError);
						}
					} else {
				...
				// On error, clear auth state directly (don't call handleSignOut which may also crash)
				try {
					await tokenManager.clearTokens();
				} catch (tokenError) {
					console.error('Error clearing tokens during init recovery:', tokenError);
				}
			...
			if (result.success) {
				...
				try {
					...
					if (userData) {
						...
						// Update Supabase auth state - wrapped defensively
						try {
							await updateSupabaseAuth();
						} catch (supabaseError) {
							console.error('Error updating Supabase auth (non-fatal):', supabaseError);
						}
						...
						try {
							...
							console.error('Error with RevenueCat initialization (non-fatal):', error);
						}
				...
				try {
					...
					if (userData) {
						...
						try {
							...
							console.error(
								'Error updating Supabase auth after sign up (non-fatal):',
								supabaseError
							);
						}
					...
					console.error('Error getting user data after sign up (non-fatal):', err);
					// Keep loading false but return the success result
				}
			}
	...
	const handleSignOut = async () => {
		...
		try {
			// Clear tokens - most critical step
			try {
				await tokenManager.clearTokens();
			} catch (tokenError) {
				console.error('Error clearing tokens (continuing sign out):', tokenError);
			}
			...
			try {
				...
				console.error('Error in authService.signOut (continuing):', signOutError);
			}

			// Always clear local auth state regardless of errors above
			setIsAuthenticated(false);
			setUser(null);
			...
			try {
				...
				console.error('Error updating Supabase auth during sign out (non-fatal):', supabaseError);
			}
			...
			try {
				...
				console.error('Error resetting RevenueCat user (non-fatal):', error);
			}
			...
			// CRITICAL: Even on error, force clear auth state to prevent crash loops
			setIsAuthenticated(false);
			setUser(null);
		} finally {
			...
			try {
				...
				if (userData) {
					...
					try {
						...
						console.error(
							'Error updating Supabase auth after Google Sign-In (non-fatal):',
							supabaseError
						);
					}
					...
					try {
						...
						console.error('Error identifying user with RevenueCat (non-fatal):', rcError);
					}
					...
					try {
						...
						console.debug('Analytics tracking error (non-fatal):', trackError);
					}
				...
				console.error('Error getting user data after Google Sign-In (non-fatal):', err);
			}
