...
export const authService = {
  ...
  signInWithApple: async (identityToken: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      console.log(
        ...
        `${MEMORO_SERVICE_URL}/auth/apple-signin`
      );
  ...
  signInWithGoogle: async (idToken: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      console.log(
        ...
        `${MEMORO_SERVICE_URL}/auth/google-signin`
      );
  ...
  isAuthenticated: async (): Promise<boolean> => {
    ...
    const { isDeviceConnected, analyzeNetworkError } = await import('~/features/errorHandling/utils/networkErrorUtils');
  ...
  getUserSettings: async (): Promise<{
    ...
    try {
      ...
      console.log('[authService] getUserSettings - Token payload:', JSON.stringify(payload, null, 2));
      console.log('[authService] getUserSettings - is_b2b value:', payload.is_b2b);
      console.log('[authService] getUserSettings - subscription_plan_id:', payload.subscription_plan_id);
