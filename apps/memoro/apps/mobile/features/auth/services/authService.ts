...
/**
 * Safely parse JSON from a response, handling cases where
 * the server returns HTML (e.g. 502/503 error pages) instead of JSON.
 */
...
export const authService = {
	...
	signIn: async (
		...
		try {
			...
			const { appToken, refreshToken } = await safeParseJson(response);
