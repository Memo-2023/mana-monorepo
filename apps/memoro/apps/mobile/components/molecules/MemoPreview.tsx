...
import ContextMenu, { ContextMenuItem } from '~/components/atoms/ContextMenu';
 ...
 */
const MemoPreviewSkeleton: React.FC<{ isDark: boolean; themeVariant: string }> = ({
	isDark,
	themeVariant,
}) => {
...
const MemoPreviewComponent: React.FC<MemoPreviewProps> = ({
	...
	// Context menu items (built below after handlers are defined)
				...
				try {
					...
					if (updatedMemo) {
						...
						const preservedStats = {
							...
							...(!updatedMemo.metadata?.stats?.audioDuration &&
	...
	const contextMenuItems: ContextMenuItem[] = [
		{
			key: 'share',
			title: t('memo_menu.share', 'Share'),
			iconName: 'share-outline',
			onSelect: onShare || undefined,
		},
		{
			key: 'copy',
			title: t('memo_menu.copy', 'Copy'),
			iconName: 'copy-outline',
			onSelect: onCopy || handleCopyAll,
		},
		{
			key: 'copy-transcript',
			title: t('memo_menu.copy_transcript', 'Copy Transcript'),
			iconName: 'document-text-outline',
			onSelect: handleCopyTranscript,
		},
		{
			key: 'pin',
			title: memo.is_pinned ? t('memo_menu.unpin', 'Unpin') : t('memo_menu.pin', 'Pin'),
			iconName: memo.is_pinned ? 'push-pin-slash' : 'push-pin',
			onSelect: onPinToTop || handlePinToggle,
		},
		{ key: 'sep-1', separator: true },
		{
			key: 'delete',
			title: t('memo_menu.delete', 'Delete'),
							...
							>
								<View className="flex-row items-center">
									...
									{currentMemo.metadata?.combined_memo_count &&
										currentMemo.metadata.combined_memo_count > 1 && (
											<>
												<Text
													variant="tiny"
													className={separatorClasses}
	...
	if (isLoading) {
		return <MemoPreviewSkeleton isDark={isDark} themeVariant={themeVariant} />;
	}
	...
	if (selectionMode) {
		return (
			<Pressable style={getContainerStyle()} onPress={() => onSelect && onSelect(!selected)}>
				<View style={{ flex: 1 }}>{renderContent()}</View>
			</Pressable>
		);
	}

	// Custom ContextMenu mit iOS-style Blur-Overlay und hervorgehobenem Element
	return (
		<ContextMenu items={contextMenuItems} onPress={onPress} style={getContainerStyle()}>
			<View style={{ flex: 1 }}>{renderContent()}</View>
		</ContextMenu>
	);
};
...
const MemoPreview = memo(MemoPreviewComponent, (prevProps, nextProps) => {
	...
	if (prevProps.memo?.timestamp?.getTime() !== nextProps.memo?.timestamp?.getTime()) return false;
	...
	if (prevProps.memo?.metadata?.stats?.editCount !== nextProps.memo?.metadata?.stats?.editCount)
		return false;
	if (
		prevProps.memo?.metadata?.stats?.audioDuration !==
		nextProps.memo?.metadata?.stats?.audioDuration
	)
		return false;
