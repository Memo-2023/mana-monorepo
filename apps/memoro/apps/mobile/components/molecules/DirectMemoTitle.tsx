 ...
 */
const DirectMemoTitle: React.FC<DirectMemoTitleProps> = memo(
	({
		memoId,
 ...
 */
const DirectMemoTitleCentralized: React.FC<DirectMemoTitleProps> = memo(
	({
		memoId,
		initialTitle = 'New Recording',
		titleClasses,
 ...
 */
const DirectMemoTitleLegacy: React.FC<DirectMemoTitleProps> = memo(
	({
		memoId,
		initialTitle = 'New Recording',
		titleClasses,
		...
		useEffect(() => {
			...
			if (!reactToGlobalRecordingStatus) {
				console.log(
					`DirectMemoTitle ${memoId}: Skipping subscription (not latest memo), using initial title`
				);
			...
			const getDisplayTitle = (memo: any): string => {
				...
				if (
					...
					simpleTranscriptionStatus === 'pending' ||
					simpleTranscriptionStatus === 'processing'
				) {
					console.log(
						`DirectMemoTitle: Transcription ${transcriptionStatus || simpleTranscriptionStatus}`
					);
				...
				if (
					(transcriptionStatus === 'completed' || simpleTranscriptionStatus === 'completed') &&
				...
				if (memo.metadata?.processing?.headline_and_intro?.status === 'completed') {
					...
					console.log(
						`DirectMemoTitle: Headline completed but no title available, showing headline generating status`
					);
					return t('memo.status.headline_generating');
				}
					...
					const subscription = supabase
						...
						.on(
							...
							(payload) => {
								...
								console.log(
									`DirectMemoTitle ${memoId}: Transcription status:`,
									payload.new.metadata?.processing?.transcription?.status
								);
								console.log(
									`DirectMemoTitle ${memoId}: Headline status:`,
									payload.new.metadata?.processing?.headline_and_intro?.status
								);
								console.log(
									`DirectMemoTitle ${memoId}: Has transcript:`,
									!!payload.new.source?.transcript
								);
								...
								if (isMounted) {
									...
									requestAnimationFrame(() => {
										if (isMounted) {
											...
											console.log(
												`DirectMemoTitle ${memoId} (Realtime): Processing update to "${newTitle}"`
											);
											updateTitleWithAnimation(newTitle);
										}
									});
