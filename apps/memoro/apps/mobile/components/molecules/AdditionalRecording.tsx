...
interface AdditionalRecordingProps {
	...
	handleUpdateSpeakerLabels?: (
		speakerMappings: Array<{ id: string; label: string }>
	) => Promise<void>;
...
export default function AdditionalRecording({
	...
	const hasAudio =
		recording.type === 'audio' && recording.path && !recording.path.startsWith('combined-memo-');
	...
	const getDisplayDate = () => {
		...
		return recording.timestamp
			? t('memo.recorded_on', 'Aufgenommen am {{date}}', {
	...
	const getTranscriptTitle = () => {
		...
		if (recording.memo_metadata?.display_title || recording.memo_metadata?.original_title) {
			const originalTitle =
				recording.memo_metadata.display_title || recording.memo_metadata.original_title;
									...
									? async (newTranscript) => {
											try {
												...
												const updatedRecordings = [
													...(memo?.metadata?.additional_recordings || []),
												];
												...
												if (error) {
													console.debug(
														'Error updating additional recording transcript:',
														error.message
													);
				...
				{!recording.transcript && recording.status === 'processing' && (
					<View style={{ marginBottom: 16, padding: 12 }}>
						<Text style={styles.statusText}>
							{t('memo.transcribing', 'Transkription wird verarbeitet...')}
						</Text>
					</View>
				)}
			</>
