...
// Module-level memo cache – survives unmount/remount (e.g. tab switches)
const memoCache = new Map<string, { memos: MemoModel[]; timestamp: number }>();
const CACHE_TTL = 5 * 60 * 1000; // 5 minutes

function getCacheKey(showArchived: boolean | null, tagIds?: string[]): string {
	const tagPart = tagIds && tagIds.length > 0 ? tagIds.slice().sort().join(',') : '';
	return `${showArchived ?? 'all'}|${tagPart}`;
}
...
interface MemoListProps {
	...
	/** Extra top padding so the list can scroll behind a transparent/absolute header */
	contentInsetTop?: number;
	/** Extra bottom padding so the list can scroll behind floating UI elements */
	contentInsetBottom?: number;
	/** Optional header component rendered at the top of the list (scrolls with content) */
...
const MemoList = ({
	...
	// Initialise from cache if available (instant display on tab switch)
	const cacheKey = getCacheKey(showArchived, tagIds);
	const cachedEntry = !propMemos ? memoCache.get(cacheKey) : undefined;
	const hasFreshCache = cachedEntry && Date.now() - cachedEntry.timestamp < CACHE_TTL;

	const [memos, setMemos] = useState<MemoModel[]>(
		propMemos || (hasFreshCache ? cachedEntry!.memos : [])
	);
	const [loading, setLoading] = useState(!propMemos && !hasFreshCache);
	const hasDisplayedData = useRef(hasFreshCache || !!propMemos);
	...
	const fetchMemos = useCallback(
		async (pageNumber: number = 0, append: boolean = false) => {
			try {
				...
				if (pageNumber === 0) {
					// Only show loading spinner if we have no data on screen yet
					if (!hasDisplayedData.current) {
						setLoading(true);
					}
				...
				if (data) {
					if (append) {
						...
						setMemos(data);
						hasDisplayedData.current = data.length > 0;

						// Update module-level cache for instant display on re-mount
						memoCache.set(getCacheKey(showArchived, tagIds), {
							memos: data,
							timestamp: Date.now(),
						});
	...
	// Subscribe to broadcast channels only for memos still being processed
	// (service_role updates bypass RLS so postgres_changes won't fire for those)
	const processingMemoIds = useMemo(() => {
		return memos
			.filter((memo) => {
				const processing = memo.metadata?.processing;
				const transcriptionDone =
					processing?.transcription?.status === 'completed' ||
					processing?.transcription?.status === 'completed_no_transcript' ||
					processing?.transcription?.status === 'error';
				const headlineDone =
					processing?.headline_and_intro?.status === 'completed' ||
					processing?.headline_and_intro?.status === 'completed_no_transcript' ||
					processing?.headline_and_intro?.status === 'error';
				// Subscribe only if still processing
				return !transcriptionDone || !headlineDone;
			})
			.map((m) => m.id);
	}, [memos]);

	useEffect(() => {
		if (processingMemoIds.length === 0) return;

		const unsubscribeFunctions: (() => void)[] = [];

		processingMemoIds.forEach((memoId) => {
			const unsubscribe = memoRealtimeService.subscribeToBroadcastChannel(
				`memo-updates-${memoId}`,
				async (payload) => {
					console.log('MemoList: Received broadcast for processing memo', memoId, payload);
					...
					try {
						...
						if (updatedMemo) {
							setMemos((prevMemos) => prevMemos.map((m) => (m.id === memoId ? updatedMemo : m)));
						}
					} catch (error) {
	...
	}, [processingMemoIds.join(',')]); // Re-subscribe only when processing memo IDs change
	...
	useEffect(() => {
		...
		const newCacheKey = getCacheKey(showArchived, tagIds);
		const cached = memoCache.get(newCacheKey);
		if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
			setMemos(cached.memos);
			hasDisplayedData.current = cached.memos.length > 0;
			setLoading(false);
			// Notify parent immediately with cached data
			if (onMemosLoaded && cached.memos.length > 0) {
				onMemosLoaded(cached.memos);
			}
		} else {
			hasDisplayedData.current = false;
		}
		...
		// Always fetch fresh data (background refresh when cache was used)
		fetchMemos(0, false);
	}, [
		showArchived,
		spaceId,
	...
	const renderMemoItem = ({ item }: { item: MemoModel }) => {
		// Never react to global recording status in the memo list
		// The home page handles showing "recording in progress" via the recording-temp preview
		// The memo list should always show actual memos with their real content
		const shouldReactToRecordingStatus = false;
		...
		<View style={styles.listWrapper}>
			...
			<FlashList
				...
				contentContainerStyle={{
					...styles.listContent,
					paddingTop: styles.listContent.paddingTop + contentInsetTop,
					paddingBottom: styles.listContent.paddingBottom + contentInsetBottom,
				}}
