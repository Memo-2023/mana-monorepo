...
const BaseLanguageSelector: React.FC<BaseLanguageSelectorProps> = ({
	...
	const themeColors = (require('../../tailwind.config.js') as any).theme?.extend?.colors as Record<
		string,
		any
	>;
	...
	const filteredLanguages = useMemo(() => {
		...
		const filtered = searchQuery.trim()
			? entries.filter(([code, language]) => {
					...
					return (
						code.toLowerCase().includes(query) || language.nativeName.toLowerCase().includes(query)
					);
				})
			: entries;
		...
		<View style={{ height, display: 'flex', flexDirection: 'column' }}>
			...
			>
				<Input
					...
					placeholder={
						searchPlaceholder ||
						t('common.search_language_count', {
				...
				>
					<Icon name="search-outline" size={20} color={textColor} />
				</View>
