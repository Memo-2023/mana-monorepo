...
export interface SatelliteButton {
	key: string;
	icon?: string;
	element?: React.ReactNode;
	onPress?: () => void;
	/** Wann soll der Button angezeigt werden? 'always' | 'recording' | 'idle' */
	showWhen?: 'always' | 'recording' | 'idle';
}
...
interface RecordingButtonProps {
	...
	spaceId?: string | null;
	blueprintId?: string | null;
	/** Externe Buttons, die radial um den Recording-Button angeordnet werden */
	satelliteButtons?: SatelliteButton[];
}
...
function RecordingButton({
	...
	// Normalized audio level for RadialWaveform (0-1)
	const [normalizedAudioLevel, setNormalizedAudioLevel] = useState(0);
	...
	// Rotations-Effekt während der Aufnahme
	// Note: isPressedDown removed from dependencies to prevent race condition where
	// button release cancels rotation before isRecording becomes true
	// Note: We only reset animations when status is STOPPED or IDLE, not when isRecording is false
	// because isRecording can be false during the async startup phase
	useEffect(() => {
		if (isRecording && !isPaused) {
			// Only start rotation if not already manually started by handlePressIn
			if (!hasManuallyStartedRotation.current) {
				console.debug('Starting continuous recording rotation from effect');
			...
			// If manually started, rotation is already running - do nothing
		} else if (isPaused) {
			...
			// Bei normalem Beenden: Rotation zurücksetzen
			console.debug('Recording stopped - resetting rotation and fill');
			hasManuallyStartedRotation.current = false;
			...
			uploadProgressAnim.value = 0;
		} else if (status === RecordingStatus.IDLE && !isRecording) {
			// Only reset when explicitly IDLE (not during startup transition)
			// This prevents resetting animations during the async recording start
			cancelAnimation(rotationAnim);
			hasManuallyStartedRotation.current = false;
			rotationAnim.value = withTiming(0, { duration: 500, easing: Easing.out(Easing.ease) });
	...
	}, [isRecording, isPaused, status]); // Removed isPressedDown and rotationAnim to prevent race conditions
	...
	useEffect(() => {
		...
		// Debug logging to trace cancel flow
		if (status === RecordingStatus.STOPPED) {
			console.debug('🔍 Recording STOPPED effect triggered:', {
				savedFile: !!savedFile,
				isCanceling: isCancelingRef.current,
				savedFileUri: savedFile?.uri?.substring(0, 50),
			});
		}
		...
		if (status === RecordingStatus.STOPPED && savedFile && !isCancelingRef.current) {
			console.debug('📤 Uploading recording (not canceled)');
			...
			// Local save was already skipped via stopRecording({ skipLocalSave: true })
			...
			},
			{
				...
				onPress: async () => {
					try {
						...
						// Stop the native recording first, skip local save since we're canceling
						// This will trigger status=STOPPED, but the flag prevents upload
						if (isRecording || isPaused) {
							await stopRecording({ skipLocalSave: true });
							console.debug('🗑️ Native recording stopped (local save skipped)');
						}
	...
	const radialFillStyle = useAnimatedStyle(() => {
		...
		return {
			...
			opacity: opacity,
			zIndex: 1, // Ensure radial fill renders above button background
		};
	});
	...
	// --- Saturn-Ring Satellite-Buttons ---
	// Buttons sitzen eng beieinander auf einer Umlaufbahn rechts vom Recording-Button
	const internalButtons: SatelliteButton[] = useMemo(() => {
	...
	}, [isRecording, isPaused, handleResumeRecording, handlePauseRecording, handleCancelRecording]);

	const visibleSatellites = useMemo(() => {
		const state = isRecording || isPaused ? 'recording' : 'idle';
		const external = satelliteButtons.filter(
			(b) => b.showWhen === 'always' || b.showWhen === state
		);
		return [...internalButtons, ...external];
	}, [internalButtons, satelliteButtons, isRecording, isPaused]);

	const satelliteSize = 48;
	const orbitRadius = size / 2 + satelliteSize / 2 + 16;
	// Winkelabstand pro Button in Grad (mehr Abstand)
	const anglePer = 28;

	// Target positions for satellites (used to drive animations)
	const satelliteTargets = useMemo(() => {
		const count = visibleSatellites.length;
		if (count === 0) return [];

		const totalSpan = (count - 1) * anglePer;
		const startAngle = -totalSpan / 2;

		return visibleSatellites.map((_, i) => {
			const angleDeg = startAngle + i * anglePer;
	...
	}, [visibleSatellites.length, orbitRadius, anglePer]);

	// Animated shared values for satellite positions and scale (max 6 slots)
	...
	// Track previous satellite keys and their slot positions
	const prevSatKeysRef = useRef<string[]>([]);

	useEffect(() => {
		const prevKeys = prevSatKeysRef.current;
		const currentKeys = visibleSatellites.map((s) => s.key);

		// Build map: old key → old slot index, and save old positions before modifying
		const oldSlotMap = new Map<string, number>();
		const oldPositions: { x: number; y: number }[] = [];
		prevKeys.forEach((key, i) => {
			oldSlotMap.set(key, i);
			oldPositions.push({ x: satX[i].value, y: satY[i].value });
		});

		for (let i = 0; i < MAX_SATELLITES; i++) {
			if (i < satelliteTargets.length) {
				const target = satelliteTargets[i];
				const key = currentKeys[i];
				const oldSlot = oldSlotMap.get(key);

				if (oldSlot !== undefined) {
					// Existing button moved to new slot: carry over old position, animate to new
					const oldPos = oldPositions[oldSlot];
					satX[i].value = oldPos.x;
					satY[i].value = oldPos.y;
					satX[i].value = withSpring(target.x, { damping: 14, stiffness: 120 });
					satY[i].value = withSpring(target.y, { damping: 14, stiffness: 120 });
					// Ensure visible
					satScale[i].value = withTiming(1, { duration: 0 });
				} else {
					// New button: snap to position, subtle scale+fade entrance
					// 0.8 → 1.08 → 1.0 with smooth bezier curves
					satX[i].value = target.x;
					satY[i].value = target.y;
					satScale[i].value = withSequence(
						withTiming(0.8, { duration: 0 }),
						withTiming(1.08, { duration: 280, easing: Easing.bezier(0.25, 0.1, 0.25, 1) }),
						withTiming(1, { duration: 180, easing: Easing.bezier(0.25, 0.1, 0.35, 1) })
					);
				}
			} else {
				// Slot not used: fade out
				satScale[i].value = withTiming(0.8, { duration: 180, easing: Easing.bezier(0.4, 0, 1, 1) });
			}
		}

		prevSatKeysRef.current = currentKeys;
	}, [visibleSatellites, satelliteTargets]);

	// Animated styles for each satellite slot
	// Position on the outer Animated.View, scale on inner Animated.View
	const satPosStyle0 = useAnimatedStyle(() => ({
		transform: [
			{ translateX: satX[0].value - satelliteSize / 2 },
			{ translateY: satY[0].value - satelliteSize / 2 },
		],
	}));
	const satPosStyle1 = useAnimatedStyle(() => ({
		transform: [
			{ translateX: satX[1].value - satelliteSize / 2 },
			{ translateY: satY[1].value - satelliteSize / 2 },
		],
	}));
	const satPosStyle2 = useAnimatedStyle(() => ({
		transform: [
			{ translateX: satX[2].value - satelliteSize / 2 },
			{ translateY: satY[2].value - satelliteSize / 2 },
		],
	}));
	const satPosStyle3 = useAnimatedStyle(() => ({
		transform: [
			{ translateX: satX[3].value - satelliteSize / 2 },
			{ translateY: satY[3].value - satelliteSize / 2 },
		],
	}));
	const satPosStyle4 = useAnimatedStyle(() => ({
		transform: [
			{ translateX: satX[4].value - satelliteSize / 2 },
			{ translateY: satY[4].value - satelliteSize / 2 },
		],
	}));
	const satPosStyle5 = useAnimatedStyle(() => ({
		transform: [
			{ translateX: satX[5].value - satelliteSize / 2 },
			{ translateY: satY[5].value - satelliteSize / 2 },
		],
	}));
	const satPosStyles = [
		satPosStyle0,
	...
	// Scale+opacity styles: subtle 0.8→1.08→1 scale with simultaneous fade 0→1
	// Opacity maps scale range [0.8, 1.0] → [0, 1], clamped
	const satScaleStyle0 = useAnimatedStyle(() => {
		'worklet';
		const s = satScale[0].value;
		return { transform: [{ scale: s }], opacity: interpolate(s, [0.8, 1.0, 1.1], [0, 1, 1]) };
	});
	const satScaleStyle1 = useAnimatedStyle(() => {
		'worklet';
		const s = satScale[1].value;
		return { transform: [{ scale: s }], opacity: interpolate(s, [0.8, 1.0, 1.1], [0, 1, 1]) };
	});
	const satScaleStyle2 = useAnimatedStyle(() => {
		'worklet';
		const s = satScale[2].value;
		return { transform: [{ scale: s }], opacity: interpolate(s, [0.8, 1.0, 1.1], [0, 1, 1]) };
	});
	const satScaleStyle3 = useAnimatedStyle(() => {
		'worklet';
		const s = satScale[3].value;
		return { transform: [{ scale: s }], opacity: interpolate(s, [0.8, 1.0, 1.1], [0, 1, 1]) };
	});
	const satScaleStyle4 = useAnimatedStyle(() => {
		'worklet';
		const s = satScale[4].value;
		return { transform: [{ scale: s }], opacity: interpolate(s, [0.8, 1.0, 1.1], [0, 1, 1]) };
	});
	const satScaleStyle5 = useAnimatedStyle(() => {
		'worklet';
		const s = satScale[5].value;
		return { transform: [{ scale: s }], opacity: interpolate(s, [0.8, 1.0, 1.1], [0, 1, 1]) };
	});
	const satScaleStyles = [
		satScaleStyle0,
		satScaleStyle1,
		satScaleStyle2,
		satScaleStyle3,
	...
	// Cleanup satellite animations on unmount
	useEffect(() => {
		return () => {
			for (let i = 0; i < MAX_SATELLITES; i++) {
				cancelAnimation(satX[i]);
				cancelAnimation(satY[i]);
				cancelAnimation(satScale[i]);
			}
		};
	}, []);

	// Drei kleine dekorative Orbit-Ringe (Platzhalter-Dots auf der linken Seite)
	const orbitDots = useMemo(() => {
		const dots = [];
		const dotAngles = [150, 180, 210]; // Links verteilt (gegenüber der Buttons)
	...
	return (
		<View style={styles.container}>
			{/* Radial Waveform - always visible, reacts to audio level during recording */}
			{!isWebPlatform && (
				<View
					style={{
						position: 'absolute',
			...
			{visibleSatellites.length > 0 && (
				<View
					style={{
						...
						top: size / 2 - orbitRadius,
						left: size / 2 - orbitRadius,
					}}
					pointerEvents="none"
				/>
			)}

			{/* Dekorative kleine Orbit-Dots (links, gegenüber der Buttons) */}
			{visibleSatellites.length > 0 &&
				orbitDots.map((dot, i) => (
					<View
						key={`orbit-dot-${i}`}
			...
			{/* Satellite-Buttons auf dem Orbit (animated) */}
			{visibleSatellites.map((sat, index) => {
				if (index >= MAX_SATELLITES) return null;

				return (
					<Animated.View
						key={`sat-slot-${index}`}
						style={[styles.satelliteButton, satPosStyles[index]]}
					>
						<Animated.View style={satScaleStyles[index]}>
							{sat.element ? (
								<View style={styles.satelliteInner}>{sat.element}</View>
							) : (
								<Pressable onPress={sat.onPress} style={styles.satelliteInner}>
									<Icon name={sat.icon || 'ellipse'} size={24} color={textColor} />
								</Pressable>
							)}
						</Animated.View>
					</Animated.View>
