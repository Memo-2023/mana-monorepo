...
import Animated, { useAnimatedStyle, useSharedValue, withTiming, withSpring, withRepeat, withSequence, Easing, cancelAnimation, interpolate } from 'react-native-reanimated';
...
interface RecordingButtonProps {
  size?: number;
  onPress?: () => void;
  onRecordingComplete?: (result: string, title?: string, spaceId?: string | null, blueprintId?: string | null, durationSeconds?: number, audioFileId?: string) => void;
...
function RecordingButton({
    ...
    if (isRecording && !isPaused && !isPressedDown) {
      ...
      if (!hasManuallyStartedRotation.current) {
        ...
        rotationAnim.value = withRepeat(
          withTiming(360 + 3600, { // Start from 360° for seamless transition
  ...
  useEffect(() => {
    ...
    if (status === RecordingStatus.STOPPED && savedFile && !isCancelingRef.current) {
      ...
      onRecordingComplete?.(savedFile.uri, title, spaceId, blueprintId, savedFile.duration, savedFile.id);
  ...
  }, [status, savedFile, onRecordingComplete, title, spaceId, blueprintId, resetRecording, loadLatestMemo]);
  ...
  const handlePressIn = useCallback(async () => {
    ...
    if (!isRecording) {
      ...
      if (!permissions.microphone.granted) {
        ...
        try {
          ...
          if (!isMountedRef.current || !isPressedDownRef.current) {
            ...
            console.debug('   ✅ Audio session initialized, stopping recording flow due to button release');
            // Reset animations
            scaleAnim.value = withSpring(1, { damping: 15, stiffness: 200 });
            setIsPressedDown(false);
            isPressedDownRef.current = false;
      ...
      halfwayTimeout.current = registerTimeout(setTimeout(() => {
        if (isPressedDownRef.current) {
          console.debug('🔔 Halfway haptic feedback');
          Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
        }
      }, PRESS_HOLD_DURATION / 2)); // 360ms - halbe Rotation

      // Timer für Recording Start nach einer vollen Drehung
      pressTimeout.current = registerTimeout(setTimeout(async () => {
      ...
      pressTimeout.current = registerTimeout(setTimeout(() => {
        if (isPressedDownRef.current) {
          ...
          if (additionalRotation > 0) {
            ...
            pressRotationAnim.value = withTiming(360 + additionalRotation, {
  ...
  }, [isRecording, isPaused, triggerStartHaptic, scaleAnim, pressRotationAnim, rotationAnim, setRecordingInfo, title, spaceId, blueprintId, startRecording, onPress, handleCompleteRecording, isPressedDown, shouldShowToastOnNextPress, showInfo, shouldShowInstructionToast, fillRadiusAnim, t, registerTimeout, initialize, reinitialize]);
  ...
  const handleStopRecording = () => {
    if (isRecording) {
      ...
      Alert.alert(
        t('recording.stop_recording_title'),
        t('recording.stop_recording_message'),
        [
          {
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.debug('🗑️ Canceling recording - setting cancel flag and stopping native recorder...');
  ...
  const styles = StyleSheet.create({
    ...
    button: {
      ...
      alignItems: 'center',
      justifyContent: 'center',
      backgroundColor: isHovered && Platform.OS === 'web' ? contentBackgroundHoverColor : contentBackgroundColor,
        ...
        <Animated.View style={[styles.buttonWrapper, buttonScaleStyle]}>
          <Animated.View
            style={[
              ...
              buttonBorderStyle,
              {
                backgroundColor: isHovered && Platform.OS === 'web' ? contentBackgroundHoverColor : contentBackgroundColor, // Hover effect on web
                borderWidth: 6,
              },
            ]}
          >
