...
import { View, Pressable, Platform, StyleSheet, TextInput } from 'react-native';
...
const Memory = forwardRef<MemoryHandle, MemoryProps>((props, ref) => {
	...
	const defaultExpanded =
		safeProps.defaultExpanded !== undefined ? safeProps.defaultExpanded : true;
		...
		() => ({
			blockquote: (node: any, children: any, parent: any, styles: any) => (
				...
				>
					...
					/>
					<View style={{ paddingLeft: 20, paddingVertical: 16, paddingRight: 16 }}>{children}</View>
				</View>
			),
	...
	const markdownTitleStyles = useMemo(
		() => createCompactMarkdownStyles(markdownStyles),
		[markdownStyles]
	);
				...
				<View style={styles.headerContent}>
					...
					{!isEditing && (
						<View style={[styles.chevron, { opacity: 0.5 }]}>
							<Icon name="chevron-down" size={16} color={isDark ? '#FFFFFF' : '#000000'} />
						</View>
					)}
				</View>
			</Pressable>
