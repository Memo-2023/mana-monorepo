...
import type { Memo, MemoSource, SpeakerUtterance } from '~/features/memos/types/memo.types';
import type { SearchResult } from '~/features/memos/hooks/useMemoSearch';

interface MemoTranscriptProps {
	memo: Memo;
	memoId: string;
	transcriptData: MemoSource | null;
	isEditMode: boolean;
	editTranscript: string;
	editUtterances: SpeakerUtterance[];
	searchQuery: string;
			...
			{audioUrl && (
				<View style={styles.audioPlayerContainer}>
					<AudioPlayer
						...
						lockScreenMetadata={{
							title: memo?.title || t('memo.audio_recording', 'Audioaufnahme'),
							artist: 'Memoro',
						}}
					/>
				</View>
