...
export default function MemoPage() {
  ...
  const scrollToTop = useCallback(() => {
    ...
    if (ref) {
      console.log('Using ref:', ref === localScrollRef.current ? 'localScrollRef' : 'scrollViewRef');
    ...
    (memoryIndex: number, sectionType: 'memory' | 'audio' | 'transcript') => {
      ...
      switch (sectionType) {
        ...
        case 'transcript':
          ...
          return headerHeight + buttonRowHeight + ((memories?.length || 0) * memoryItemHeight) + audioSectionHeight + 50;
        default:
          return 0;
      }
    },
  ...
  const scrollToTranscript = useCallback(() => {
    ...
    if (ref && ref.scrollTo) {
      const refName = ref === localScrollRef.current ? 'localScrollRef' :
                     ref === globalScrollRef.current ? 'globalScrollRef' : 'scrollViewRef';
  ...
  useEffect(() => {
    ...
    if (memo?.title) {
      updateConfig({
        ...
        showBackButton: true,
        showMenu: true, // Enable the HeaderMenu like on all other pages
  ...
  return (
    <>
      ...
      {/* Edit Toolbar - Außerhalb von KeyboardAvoidingView mit manueller Positionierung */}
      {isEditMode && (
        <View
          style={{
            position: 'absolute',
      ...
      {/* Prompt Bar - Außerhalb von KeyboardAvoidingView mit manueller Positionierung */}
      {showPromptBar && (
        <View
          style={{
            position: 'absolute',
          ...
          <PromptBar
            onSubmit={async (prompt) => {
              ...
              setPromptValue('');
              // Nach dem Absenden die PromptBar ausblenden
              setShowPromptBar(false);
              setPromptInputFocus(false);
            }}
            placeholder={
