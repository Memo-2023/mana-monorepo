...
export default function Memos() {
  ...
  const params = useLocalSearchParams<{ autoUpload?: string; selectedTagIds?: string; openUploadModal?: string }>();
  ...
  // Helper for selection toolbar visibility
  const isToolbarVisible = isSelectionMode && selectedMemoIds.length > 0;
  ...
  const processUploadedFile = async (
    ...
    try {
      ...
      if (!fileExtension || (!audioExtensions.includes(fileExtension) && !videoExtensions.includes(fileExtension))) {
  ...
  // Überprüfen der Supabase-Verbindung und Authentifizierung
  useEffect(() => {
    const checkSupabaseConnection = async () => {
      try {
        ...
        if (error) {
          ...
          // Removed debug log for successful database connection
        }
      } catch (error) {
        // Removed debug log for Supabase connection error
      }
    };
  ...
  const updateHeaderWithSelectedTags = (tagIds: string[]) => {
    ...
    updateConfig({
      title: 'Memos',
      showTitle: true, // Explizit setzen, um Memo-Detail-Seite zu überschreiben
      showBackButton: false,
      showMenu: true,
      rightIcons: getHeaderRightIcons(),
  ...
  // Hilfsfunktion, um die rechten Header-Icons zu erhalten
  const getHeaderRightIcons = () => {
    return [
      {
      ...
      {
        name: 'waveform',
        href: '/(protected)/audio-archive',
      },
      {
        name: isUploading ? 'hourglass-outline' : 'cloud-upload-outline',
        onPress: isUploading ? undefined : () => setIsUploadModalVisible(true),
      },
    ];
  };
  ...
  const styles = StyleSheet.create({
    listContainer: {
      flex: 1,
      paddingBottom: 60, // Reduced padding as PromptBar is removed, only PillFilter remains
    },
    uploadingContainer: {
      position: 'absolute',
      top: 10,
    ...
    toolbarContainer: {
      position: 'absolute',
      bottom: 60, // Adjusted to sit directly above PillFilter
      left: 0,
      right: 0,
      zIndex: 20,
      pointerEvents: 'box-none', // Allow mouse events to pass through
  ...
  return (
    <>
      ...
      <View style={{ flex: 1, backgroundColor: pageBackgroundColor, position: 'relative' }}>
        <View
          style={[
            ...
            // Add extra bottom padding when toolbar is visible
            isToolbarVisible && { paddingBottom: 140 },
          ]}>
          <LoadingOverlay
            visible={isUploading}
        ...
        {isToolbarVisible && (
          <View style={styles.toolbarContainer}>
            <Toolbar
              ...
              backgroundColor={
                isDark
                  ? (colors as any).theme?.extend?.colors?.dark?.[themeVariant]?.menuBackground
                  : (colors as any).theme?.extend?.colors?.[themeVariant]?.menuBackground
              }
              primaryActions={[
                {
                ...
                // { icon: 'folder-outline', label: 'Space', onPress: handleOpenSpaceSelector, variant: 'secondary' },
                {
                  icon: 'git-merge-outline',
                  label: t('memo.combine', 'Kombinieren'),
                  onPress: () => setIsCombineModalVisible(true),
                  variant: 'secondary',
                },
                {
                  icon: 'trash-outline',
        ...
        <View style={styles.pillFilterContainer}>
          <TagPillFilter selectedTagIds={selectedTagIds} onTagSelect={handleTagSelect} />
        </View>

        {/* Search Button */}
        {!isSelectionMode && allMemos.length > 0 && (
          <View
            style={{
              position: 'absolute',
              bottom: insets.bottom + 40, // Adjusted spacing above pill filter
              right: 20,
              zIndex: 1000,
            }}>
            <ActionButton
      ...
      {/* Search Overlay - Outside of container view like in tags page */}
      <SearchOverlay
