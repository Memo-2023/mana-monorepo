...
const Statistics: React.FC = () => {
  ...
  const loadStatistics = async () => {
    ...
    try {
      ...
      memos.forEach(memo => {
        ...
        if (!wordCount) {
          const transcript = memo.source?.transcript || memo.source?.content || memo.source?.transcription || '';
          if (transcript) {
        ...
        if (lastViewed && (!lastViewedMemo || new Date(lastViewed) > new Date(lastViewedMemo.lastViewed))) {
          lastViewedMemo = {
            id: memo.id,
            title: memo.title || memo.intro || t('memo.untitled'),
            lastViewed
      ...
      const { currentStreak, longestStreak, activestWeek, activestMonth } = calculateActivityMetrics(memos, t);
      ...
      const memosWithAudio = memos.filter(memo => (memo.metadata?.stats?.audioDuration || 0) > 0).length;
      ...
      const stats: StatisticsData = {
        ...
        averageAudioDuration: memosWithAudio > 0 ? Math.round(totalAudioDuration / memosWithAudio) : 0,
        ...
        averageWordsPerMinute: totalRecordingDuration > 0 ? Math.round((totalWordsInRecordings / (totalRecordingDuration / 60)) * 10) / 10 : 0,
  ...
  const calculateActivityMetrics = (memos: any[], t: any) => {
    ...
    // Sort memos by date
    const sortedMemos = memos.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
    ...
    const activestWeek = Object.entries(weekCounts).reduce((max, [week, count]) => 
      count > max.count ? { week, count } : max, { week: t('statistics.no_data_week'), count: 0 });
    
    const activestMonth = Object.entries(monthCounts).reduce((max, [month, count]) => 
      count > max.count ? { month, count } : max, { month: t('statistics.no_data_month'), count: 0 });
  ...
  const calculateTagAnalytics = (memos: any[], memoTags: any[], tags: any[]) => {
    ...
    const averageTagsPerMemo = memos.length > 0 ? Math.round((totalTagAssignments / memos.length) * 10) / 10 : 0;
