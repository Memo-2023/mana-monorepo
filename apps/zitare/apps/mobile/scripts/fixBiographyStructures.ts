		...
		if (inBiography) {
			...
			if (trimmed.startsWith('"short":')) {
				...
				if (i + 1 < lines.length) {
					...
					const isSectionStart = nextLine.match(
						/^"(teacher|politician|wanderer|basel|weimar|exile|return|master|zarathustra|madness|earlyLife|patentClerk|riseToProminence|breakthrough|finalYears|earlyCareer|vienna|rome|composer|parisian|salon|court|renaissance|artist|polymath|warrior|philosopher|enlightenment|minister)": \{/
					);
