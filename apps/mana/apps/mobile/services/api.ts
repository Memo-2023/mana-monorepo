...
const BASE_URL = process.env.EXPO_PUBLIC_MANA_AUTH_URL || 'http://localhost:3001';

/**
 * API client for mana-auth service.
 * Wraps fetch calls and automatically attaches JWT from SecureStore.
 */
class ManaApi {
	private async getToken(): Promise<string | null> {
		try {
			const raw = await SecureStore.getItemAsync('@mana/app_token');
			if (!raw) return null;
