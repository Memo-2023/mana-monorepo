...
<RoutePage appId="presi" backHref="/presi" title="Präsentation">
		...
		{#if currentSlide}
			...
			>
				...
				<button
					...
					class="p-2 hover:bg-muted/10 rounded-lg transition-colors"><X class="w-6 h-6" /></button
				>
			</div>
				...
				<div class="max-w-4xl mx-auto flex items-center justify-between">
					...
					<div class="flex items-center gap-2">
						...
						<button
							...
							class="p-2 hover:bg-muted/10 rounded-lg transition-colors"
						>
							{#if isFullscreen}<ArrowsIn class="w-5 h-5" />{:else}<ArrowsOut
									class="w-5 h-5"
								/>{/if}
						</button>
					</div>
