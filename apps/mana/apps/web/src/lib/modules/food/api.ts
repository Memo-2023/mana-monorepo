...
export async function uploadMealPhoto(file: File): Promise<UploadMealPhotoResult> {
	...
	const res = await fetch(`${getManaApiUrl()}/api/v1/food/photos/upload`, {
		method: 'POST',
...
export async function analyzeMealPhoto(photoUrl: string): Promise<MealAnalysisResult> {
	const res = await fetch(`${getManaApiUrl()}/api/v1/food/analysis/photo`, {
		method: 'POST',
		headers: {
...
export async function analyzeMealText(description: string): Promise<MealAnalysisResult> {
	const res = await fetch(`${getManaApiUrl()}/api/v1/food/analysis/text`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			...(await authHeader()),
