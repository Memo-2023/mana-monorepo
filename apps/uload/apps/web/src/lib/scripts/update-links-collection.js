...
async function updateLinksCollection() {
    try {
        ...
        await pb.collections.update('links', {
            ...
            listRule: "user_id = @request.auth.id || created_by = @request.auth.id || account_owner = @request.auth.id || is_active = true",
            viewRule: "user_id = @request.auth.id || created_by = @request.auth.id || account_owner = @request.auth.id || is_active = true",
            updateRule: "created_by = @request.auth.id || (account_owner = @request.auth.id && created_by = @request.auth.id)",
            deleteRule: "created_by = @request.auth.id || (account_owner = @request.auth.id && created_by = @request.auth.id)"
        });
