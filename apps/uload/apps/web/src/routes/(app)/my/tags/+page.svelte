<script lang="ts">
	...
	async function bulkDelete() {
		...
		if (!confirm(`Möchten Sie wirklich ${selectedTags.size} Tag(s) löschen? Alle Verknüpfungen zu Links werden entfernt.`)) {
			return;
		}
			...
			<div class="mb-4 flex items-center gap-4 rounded-lg border border-theme-border bg-theme-surface p-4">
				<label class="flex items-center gap-2 cursor-pointer">
					...
					<span class="text-theme-text font-medium">
						{selectedTags.size === filteredAndSortedTags.length ? 'Alle abwählen' : 'Alle auswählen'}
					</span>
				</label>
			...
			<div class="space-y-4">
				...
				<div class="flex gap-2">
					<button
						...
						class="flex-1 rounded-lg bg-theme-primary px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-theme-primary-hover disabled:opacity-50 disabled:cursor-not-allowed"
					>
						Tags zusammenführen
					</button>
					<button
