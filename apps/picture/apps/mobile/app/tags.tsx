...
export default function TagsScreen() {
  ...
  const renderTag = ({ item }: { item: Tag }) => (
    <View style={{ backgroundColor: theme.colors.surface, borderRadius: 8, padding: 16, marginBottom: 12, marginHorizontal: 16 }}>
      <View className="flex-row items-center justify-between">
        <View className="flex-row items-center flex-1">
          ...
          <View className="flex-1">
            <Text variant="body" color="inverse" weight="medium">#{item.name}</Text>
  ...
  const ColorPicker = ({ selectedColor, onColorChange }: { selectedColor: string, onColorChange: (color: string) => void }) => (
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      {isLoading && !refreshing ? (
        <View className="flex-1 justify-center items-center">
          ...
          <Text variant="body" color="secondary" className="mt-2">Lade Tags...</Text>
        </View>
      ) : (
        <FlatList
          data={tags}
      ...
      {error && (
        <View className="absolute bottom-20 left-4 right-4 bg-red-600 rounded-lg p-3">
          <Text variant="body" color="inverse" align="center">{error}</Text>
        </View>
      )}
      ...
      >
        <View className="flex-1 bg-black/50 justify-end">
          <View style={{ backgroundColor: theme.colors.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24 }}>
            ...
            <Text variant="label" color="secondary" className="mb-2">Tag-Name</Text>
            <TextInput
              value={newTagName}
              onChangeText={setNewTagName}
              placeholder="Tag-Name eingeben..."
              placeholderTextColor="#6b7280"
              style={{ backgroundColor: theme.colors.input, borderWidth: 1, borderColor: theme.colors.border, borderRadius: 8, paddingHorizontal: 16, paddingVertical: 12, color: theme.colors.text.primary, marginBottom: 16 }}
              autoCapitalize="none"
            />

            <Text variant="label" color="secondary" className="mb-2">Farbe wählen</Text>
            <ColorPicker 
