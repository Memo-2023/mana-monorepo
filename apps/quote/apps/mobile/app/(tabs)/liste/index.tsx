...
export default function Liste() {
	...
	// Premium store
	const { canCreateCollection, getRemainingCollections, MAX_WEEKLY_COLLECTIONS } =
		usePremiumStore();
	...
	const { lists, initializeLists, updateList, deleteList, duplicateList, getListStats } =
		useListStore();
	...
	const createEmptyList = React.useCallback(
		(): List => ({
			id: 'new-list-temp',
	...
	const renderListCard = ({ item, index }: { item: List; index: number }) => {
		...
		return (
			<View style={wrapperStyle} className={viewMode === 'list' ? LIST_ITEM_CLASSES.wrapper : ''}>
				{!isEditing ? (
					<Pressable
						onPress={() => {
							Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
						...
						onLongPress={() => {
							Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
							if (!item.isDefault) {
								Alert.alert(item.name, t('lists.listActions'), [
									{ text: t('common.cancel'), style: 'cancel' },
						...
						>
							<View className="bg-black/40 rounded-3xl backdrop-blur-xl">
								<View className="p-6">
									...
									{item.description && (
										<Text className="text-white/60 mb-4" style={{ fontSize: 14, lineHeight: 20 }}>
											{item.description}
										</Text>
									)}
					...
					<LinearGradient
						colors={
							isDarkMode
								? [newListData.color, newListData.color + 'CC']
								: [newListData.color + 'CC', newListData.color + '99']
						}
						start={{ x: 0, y: 0 }}
						end={{ x: 1, y: 1 }}
						style={{
					...
					>
						<View className="bg-black/40 rounded-3xl backdrop-blur-xl">
							<View className="p-6">
								...
								<View className={`border-t border-white/10 pt-3`}>
									<View className="flex-row items-center justify-between">
										<Text className="text-white/40 text-sm">{t('lists.newList')}</Text>
