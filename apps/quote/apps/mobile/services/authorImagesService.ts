...
export function enrichAuthorWithImage<T extends { id: string }>(author: T): T & { imageUrl?: string } {
  const imageUrl = getAuthorImageUrl(author.id);
  return imageUrl ? { ...author, imageUrl } : author;
}

export function enrichAuthorsWithImages<T extends { id: string }>(authors: T[]): (T & { imageUrl?: string })[] {
