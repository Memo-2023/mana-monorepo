...
async function testJSONLoading() {
	try {
		...
		console.log(
			`   ✅ German: ${deQuotes.quotes.length} quotes, ${deAuthors.authors.length} authors`
		);
		console.log(
			`   ✅ English: ${enQuotes.quotes.length} quotes, ${enAuthors.authors.length} authors`
		);
		...
		if (deQuote && enQuote) {
			...
			console.log(
				`   📊 Translation: ${deQuote.text !== enQuote.text ? '✅ Different' : '❌ Same'}`
			);
		}
		...
		try {
			...
			setTimeout(() => {
				...
				console.log(
					`   🇩🇪 German: ${deStats.totalQuotes} quotes, ${deStats.totalAuthors} authors, ${deStats.totalCategories} categories`
				);
				...
				console.log(
					`   🇺🇸 English: ${enStats.totalQuotes} quotes, ${enStats.totalAuthors} authors, ${enStats.totalCategories} categories`
				);
