...
async function main() {
  ...
  console.log(`📚 Processing ${authorsToProcess.length} authors (batch ${Math.floor(startIndex/batchSize) + 1}):`);
  ...
  const totalVerifiedAuthors = archiveAuthors.filter(author => author.verified !== false && author.id !== 'unbekannt').length;
