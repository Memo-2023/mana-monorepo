...
function deduplicateQuotes(filePath: string, language: 'de' | 'en') {
  ...
  const importStatement = fileContent.match(/^import .*?;/m)?.[0] || "import { EnhancedQuote } from '../../contentLoader';";
...
const deResult = deduplicateQuotes(
...
const enResult = deduplicateQuotes(
