...
function removeDuplicatesManually(filePath: string): number {
  ...
  if (filePath.includes('de.ts')) {
    const duplicatesToRemove = [
      { id: 'aristoteles', startPattern: '  {\n    "id": "aristoteles",\n    "name": "Aristoteles",\n    "profession": [\n      "Philosoph"', lineCount: 15 },
      { id: 'aurelius-marcus', startPattern: '  {\n    "id": "aurelius-marcus",', lineCount: 12 },
      { id: 'brecht-bertolt', startPattern: '  {\n    "id": "brecht-bertolt",\n    "name": "Bertolt Brecht",\n    "profession": [\n      "Playwright"', lineCount: 12 },
      { id: 'hesse-hermann', startPattern: '  {\n    "id": "hesse-hermann",\n    "name": "Hermann Hesse",\n    "profession": [\n      "Writer"', lineCount: 12 },
      { id: 'roosevelt-eleanor', startPattern: '  {\n    "id": "roosevelt-eleanor",\n    "name": "Eleanor Roosevelt",\n    "profession": [\n      "First Lady"', lineCount: 15 }
    ];
  ...
  if (filePath.includes('en.ts')) {
    ...
    const idsToRemove = ['aristoteles', 'aurelius-marcus', 'demokrit', 'drucker-peter', 
                         'ford-henry', 'hesse-hermann', 'lasorda-tommy', 'roosevelt-eleanor', 
                         'sokrates', 'swindoll-charles'];
...
async function main() {
  ...
  console.log(`DE: ${deBefore.total} total, ${deBefore.unique} unique (${deBefore.total - deBefore.unique} duplicates)`);
  console.log(`EN: ${enBefore.total} total, ${enBefore.unique} unique (${enBefore.total - enBefore.unique} duplicates)`);
  ...
  console.log(`DE: ${deAfter.total} total, ${deAfter.unique} unique (${deAfter.total - deAfter.unique} duplicates)`);
  console.log(`EN: ${enAfter.total} total, ${enAfter.unique} unique (${enAfter.total - enAfter.unique} duplicates)`);
