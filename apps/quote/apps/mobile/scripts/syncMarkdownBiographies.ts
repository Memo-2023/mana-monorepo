...
function parseMarkdownContent(content: string): AuthorBiography {
  ...
  if (quoteIndex !== -1) {
    biography.famousQuote = lines[quoteIndex].replace(/^>\s*"?/, '').replace(/"?$/, '').trim();
  }
...
function updateAuthorData(authorId: string, biography: AuthorBiography, filePath: string): boolean {
  try {
    ...
    const biographyObj = {
      ...
      ...(biography.keyAchievements && biography.keyAchievements.length > 0 && { keyAchievements: biography.keyAchievements }),
