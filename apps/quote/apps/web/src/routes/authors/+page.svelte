<script lang="ts">
  import { authorsDE, quotesDE, type Author } from '@quote/shared';
  import AuthorCard from '$lib/components/AuthorCard.svelte';

  // Get quote counts for each author
  const authorsWithQuotes = authorsDE.map(author => {
  ...
  let filteredAuthors = $derived(
    allFilteredAuthors.slice(0, currentPage * ITEMS_PER_PAGE)
  );
...
<svelte:head>
  <title>Authors - Quotes Web App</title>
  <meta name="description" content="Browse all authors and their quotes" />
</svelte:head>
