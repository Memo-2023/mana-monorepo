<script lang="ts">
  ...
  import { authorsDE, quotesDE } from '@quote/shared';
        ...
        <div class="quotes-list">
          {#if authorQuotes.length > 0}
            {#each authorQuotes as quote (quote.id)}
              <QuoteCard
                {quote}
                on:toggleFavorite={handleQuoteFavoriteToggle}
              />
            {/each}
          {:else}
        ...
        <div class="biography">
          ...
          {#if author.biography?.famousQuote}
            <div class="bio-section famous-quote">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor" opacity="0.4">
