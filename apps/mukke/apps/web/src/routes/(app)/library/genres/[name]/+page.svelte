<script lang="ts">
	...
	let genreName = $derived(decodeURIComponent($page.params.name ?? ''));
	...
	$effect(() => {
		...
		fetchApi<{ songs: Song[] }>(`/library/genres/${encodeURIComponent(decodeURIComponent(name))}`)
			.then((data) => {
				songs = data.songs;
			})
			.catch((e) => {
...
<div class="p-6 max-w-4xl mx-auto">
	...
	{:else}
		<!-- Genre header -->
		<div class="mb-8">
			<h1 class="text-3xl font-bold mb-1">{genreName}</h1>
