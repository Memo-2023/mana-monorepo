...
async function checkStoriesSchema() {
  ...
  try {
    ...
    if (columns && columns.length > 0) {
      ...
      console.log('\n📋 Column checks:');
      console.log(`  ✓ is_favorite: ${columnNames.includes('is_favorite') ? '✅ EXISTS' : '❌ MISSING'}`);
      console.log(`  ✓ is_published: ${columnNames.includes('is_published') ? '✅ EXISTS' : '❌ MISSING'}`);
      console.log(`  ✓ visibility: ${columnNames.includes('visibility') ? '✅ EXISTS' : '❌ MISSING'}`);
