...
import { PipelineStep, PipelineContext, PipelineResult, ExecutionLog } from './pipeline.types';
...
export class PipelineExecutor {
	...
	async execute<T>(
		...
		try {
			...
			this.logger.error(`Pipeline failed at step: ${executedSteps.length}`, error);

			// Execute rollbacks in reverse order
			await this.executeRollbacks(executedSteps.reverse(), currentData, error as Error, context);
	...
	async executeStep(
		...
		try {
			...
			let output;
			if (step.timeout) {
				output = await this.executeWithTimeout(step.execute(input, context), step.timeout);
			} else {
	...
	private async retryStep(
		...
		for (let attempt = 1; attempt <= maxRetries; attempt++) {
			this.logger.log(`Retry attempt ${attempt}/${maxRetries} for step: ${step.name}`);
