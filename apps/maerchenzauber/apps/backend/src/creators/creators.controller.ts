...
export class CreatorsController {
	...
	async getCreators() {
		try {
			...
			throw new HttpException('Failed to fetch creators', HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	async getCreator(@Param('id') id: string) {
		try {
			...
			throw new HttpException('Failed to fetch creator', HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	@Post()
	@UseGuards(AuthGuard)
	async createCreator(@Body() createDto: CreateCreatorDto, @CurrentUser() user: JwtPayload) {
		...
		try {
			...
			throw new HttpException('Failed to create creator', HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	async updateCreator(
		...
		try {
			...
			if (updateDto.systemPrompt !== undefined) updateData.system_prompt = updateDto.systemPrompt;
	...
	@Delete(':id')
	@UseGuards(AuthGuard)
	async deleteCreator(@Param('id') id: string, @CurrentUser() user: JwtPayload) {
		// Check admin permission
		if (!this.isAdmin(user)) {
			throw new HttpException('Unauthorized', HttpStatus.FORBIDDEN);
		}
