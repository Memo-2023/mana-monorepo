...
export class PublicCharacterController {
  ...
  async voteForCharacter(
    ...
    try {
      ...
      const character = await this.supabaseAuthService.getPublicCharacterById(
        body.characterId,
      );
  ...
  async removeVote(
    ...
    this.logger.log(
      `User ${user.sub} removing vote from character ${characterId}`,
    );

    try {
      const result = await this.supabaseAuthService.removeVote(
        characterId,
        user.sub,
        token,
      );
  ...
  async cloneCharacter(
    ...
    try {
      ...
      const publicCharacter =
        await this.supabaseAuthService.getPublicCharacterById(characterId);
  ...
  async getCharacterByIdAndShareCode(
    ...
    this.logger.log(
      `Fetching character ${id} with share code: ${shareCode} (no auth required)`,
    );

    try {
      // Use service account to fetch character (bypasses RLS)
      const character =
        await this.supabaseAuthService.getSharedCharacter(id);
      ...
      const transformedCharacter = {
        ...
        name: character.name,
        imageUrls:
          character.images_data?.map((img: any) => img.image_url) || [
            character.image_url,
          ],
        description:
          character.user_description || character.character_description,
  ...
  async getCharacterByShareCode(@Param('shareCode') shareCode: string) {
    ...
    try {
      const character = await this.supabaseAuthService.getCharacterByShareCode(
        shareCode,
      );
