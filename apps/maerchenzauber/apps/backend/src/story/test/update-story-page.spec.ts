...
describe('StoryService - updateStoryPageText', () => {
	...
	describe('updateStoryPageText', () => {
		it('should update story text for a valid page', () => {
			...
			expect(result.data?.[1].story_text).toBe('Updated German text for page 2');
			// Other pages should remain unchanged
			expect(result.data?.[0].story_text).toBe('Once upon a time in German');
			expect(result.data?.[2].story_text).toBe('And then something happened in German');
		});
		...
		it('should preserve other page properties when updating', () => {
			const result = service.updateStoryPageText(mockPagesData, 2, undefined, 'New text');
		...
		it('should return error for invalid pages data', () => {
			const result = service.updateStoryPageText(null as any, 1, undefined, 'text');
		...
		it('should return error for non-existent page number', () => {
			const result = service.updateStoryPageText(mockPagesData, 99, undefined, 'text');
		...
		it('should update both text fields if both are provided', () => {
			const result = service.updateStoryPageText(mockPagesData, 1, 'English text', 'German text');
