...
import { STORY_RESPONSE_FORMAT_GERMAN, StoryResponse, StoryResponsePage } from '../models/story';
		...
		try {
           ...
           Geschichte: ${text}`,
			...
			try {
				...
				if (!content) {
					throw new Error('Content is null or undefined (likely content filter)');
				}
				parsedResponse = JSON.parse(content);
			} catch (parseError) {
				console.error('[translateToGerman] JSON parsing error:', parseError);
				console.error('[translateToGerman] Raw content received:', JSON.stringify(response.data));
			...
			if (!parsedResponse || !parsedResponse.pages || !Array.isArray(parsedResponse.pages)) {
