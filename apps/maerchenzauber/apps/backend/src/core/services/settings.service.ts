...
export class SettingsService implements OnModuleInit {
  ...
  private replicateModel: string =
    IMAGE_MODELS[DEFAULT_IMAGE_MODEL].replicateId; // Default to imagen-4-fast
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      if (creatorsError) {
        console.error(
          'Failed to fetch creators from Märchenzauber Supabase:',
          creatorsError,
        );
        ...
        this.creators = creatorsData.map((creator) => ({
          ...
          systemPrompt:
            ...
            (creator.type === 'author'
              ? this.defaultAuthorPrompt
              : this.defaultIllustratorPrompt),
        ...
        console.log(
          `Loaded ${this.creators.length} creators from Märchenzauber database:`,
        );
        this.creators.forEach((c) =>
          console.log(`  - ${c.name} (${c.type}): ${c.creatorId}`),
        );
      }
      ...
      console.error(
        'Failed to initialize settings from Märchenzauber Supabase:',
        error,
      );
    ...
    try {
      ...
      if (settings) {
        settings.forEach((setting) => {
          if (
            setting.image_model &&
            IMAGE_MODELS[setting.image_model as ImageModelId]
          ) {
            this.userImageModels.set(
              setting.user_id,
              setting.image_model as ImageModelId,
            );
          }
        });
  ...
  public getAuthorPrompt(creatorId?: string | null): string {
    ...
    if (!creator) {
      console.info(
        `Creator with id ${creatorId} not found, using default author prompt`,
      );
      return this.defaultAuthorPrompt;
    }
  ...
  public getIllustratorPrompt(creatorId?: string | null): string {
    ...
    if (!creator) {
      console.info(
        `Creator with id ${creatorId} not found, using default illustrator prompt`,
      );
      return this.defaultIllustratorPrompt;
    }
    return creator.systemPrompt;
  ...
  public getAuthorPromptWithExtra(creatorId?: string | null): ExtraPrompts {
    if (!creatorId) {
      console.info(
        'No creator ID provided, using default author extra prompts',
      );
      return {
    ...
    return {
      extraPromptBeginning: creator.extraPromptBeginning
        ? creator.extraPromptBeginning
        : '',
      extraPromptEnd: creator.extraPromptEnd ? creator.extraPromptEnd : '',
    };
  }

  public getIllustratorPromptWithExtra(
    creatorId?: string | null,
  ): ExtraPrompts {
    if (!creatorId) {
      console.info(
        'No creator ID provided, using default illustrator extra prompts',
      );
      return {
        extraPromptBeginning: '',
        extraPromptEnd: '',
  ...
  public async getUserImageModel(
    userId: string,
  ): Promise<Result<ImageModelId>> {
    try {
      const supabase = this.supabaseProvider.getClient();
  ...
  public async setUserImageModel(
    userId: string,
  ...
  public formatPrompt(
    ...
    return template.replace(/\{([^}]+)\}/g, (match, path) => {
      const value = path
        ...
        .reduce(
          (obj: any, key: any) =>
            obj && obj[key] !== undefined ? obj[key] : match,
          variables,
        );
      return String(value);
    });
