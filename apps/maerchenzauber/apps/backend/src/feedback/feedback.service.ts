...
export class FeedbackService {
	...
	async getPublicFeedback(userId: string, query: FeedbackQueryDto) {
		...
		// Build query
		let feedbackQuery = supabase.from('user_feedback').select('*').eq('is_public', true);
	...
	async voteFeedback(userId: string, feedbackId: string) {
		...
		console.log(
			`[FeedbackService] Vote added for feedback ${feedbackId}. New count: ${newVoteCount}`
		);
	...
	async unvoteFeedback(userId: string, feedbackId: string) {
		...
		console.log(
			`[FeedbackService] Vote removed for feedback ${feedbackId}. New count: ${newVoteCount}`
		);
