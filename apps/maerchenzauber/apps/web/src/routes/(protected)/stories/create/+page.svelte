	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/90 backdrop-blur">
		<div class="flex flex-col items-center gap-6 p-8 text-center">
			...
			<div>
				...
				<p class="text-white/70">Die Magie arbeitet! Dies kann bis zu 2 Minuten dauern.</p>
			</div>
...
<div class="mx-auto max-w-4xl space-y-8">
	...
	<section class="space-y-4">
		...
		{:else if characters.length === 0}
			<!-- No Characters -->
			<div
				class="rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 p-8 text-center dark:border-gray-700 dark:bg-gray-800/50"
			>
				<svg
			...
			<div class="flex flex-wrap gap-4">
				{#each characters.filter((c) => !c.archived) as character (character.id)}
					<button
						...
						class="flex flex-col items-center gap-2 rounded-2xl p-3 transition-all {selectedCharacter?.id ===
						character.id
					...
					>
						<div class="relative">
							<div
								class="h-20 w-20 overflow-hidden rounded-full border-3 border-white shadow-md dark:border-gray-700"
							>
								<img
							...
							{#if selectedCharacter?.id === character.id}
								<div
									class="absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full bg-pink-500 text-white"
								>
									<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										<path
											stroke-linecap="round"
						...
						<span
							class="max-w-[100px] truncate text-sm font-medium text-gray-700 dark:text-gray-300"
						>
							{character.name}
						</span>
					</button>
	...
	<section class="space-y-4">
		...
		<div class="space-y-2">
			...
			></textarea>
			<p class="text-sm text-gray-500 dark:text-gray-400">
				Beschreibe den Handlungsablauf, die Stimmung und wichtige Ereignisse. Die KI wird daraus
				eine passende Geschichte erstellen und illustrieren.
			</p>
		</div>
	</section>
