...
>
	...
	{#if !isFullscreen}
		<header
			class="absolute left-0 right-0 top-0 z-10 flex items-center justify-between bg-gradient-to-b from-black/80 to-transparent p-4"
		>
			<!-- Back Button -->
			<button
				onclick={(e) => {
			...
			<div class="flex items-center gap-2">
				{#if onToggleFavorite}
					...
					>
						<svg
							class="h-5 w-5"
							fill={story.is_favorite ? 'currentColor' : 'none'}
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
							<path
	...
	<div class="flex flex-1 items-center justify-center overflow-hidden">
		...
		{:else if currentPage === story.pages.length}
			<!-- End Screen -->
			<StoryEndScreen {onClose} onRestart={restart} {onArchive} />
		{:else}
			<!-- Story Page -->
			<StoryPageView
		...
		>
			<div class="flex items-center justify-center gap-2">
				...
				<button
					...
					class="h-2 rounded-full transition-all {currentPage === -1
						? 'w-4 bg-white'
						: 'w-2 bg-white/40'}"
					title="Titelseite"
				></button>
				...
				{#each story.pages as _, index}
					<button
						...
						class="h-2 rounded-full transition-all {currentPage === index
							? 'w-4 bg-white'
							: 'w-2 bg-white/40'}"
						title="Seite {index + 1}"
					></button>
				...
				<button
					onclick={(e) => {
						e.stopPropagation();
						goToPage(story.pages.length);
					}}
					class="h-2 rounded-full transition-all {currentPage === story.pages.length
						? 'w-4 bg-white'
						: 'w-2 bg-white/40'}"
					title="Ende"
				></button>
