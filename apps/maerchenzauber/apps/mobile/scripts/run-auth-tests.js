...
function formatResults(exitCode, duration, suite) {
	...
	console.log(
		`${colors.bright}Status:${colors.reset}    ${statusColor}${status}${colors.reset} ${icon}`
	);
...
function generateReport(options, results) {
	...
	try {
		...
		console.error(
			`${colors.yellow}Warning: Could not save test report: ${error.message}${colors.reset}`
		);
	}
}
...
function runPreTestChecks() {
	...
	if (!fs.existsSync(jestPath)) {
		console.error(
			`${colors.red}Error: Jest not found. Please run 'npm install' first.${colors.reset}`
		);
		process.exit(1);
	}
...
`);
	...
	if (options.debug) {
		console.log(
			`${colors.yellow}🐛 Debug mode enabled - detailed logging will be shown${colors.reset}`
		);
		console.log(
			`${colors.dim}Debugging utilities will be available during test execution${colors.reset}\n`
		);
	}
	...
	return new Promise((resolve) => {
		...
		child.on('close', (exitCode) => {
			...
			if (options.coverage && exitCode === 0) {
				...
				console.log(
					`${colors.dim}Open coverage/auth/lcov-report/index.html to view detailed coverage${colors.reset}`
				);
			}
