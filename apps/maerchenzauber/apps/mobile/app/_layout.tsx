...
function RootLayoutNav() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      }, Platform.OS === 'ios' ? 4000 : 600); // Increased timeout for iOS to prevent image deallocation crashes
  ...
  useEffect(() => {
    ...
    if (isSameTarget && timeSinceLastNav < navigationCooldown) {
      console.log(`Navigation to ${targetRoute} blocked (cooldown: ${navigationCooldown - timeSinceLastNav}ms remaining)`);
      return;
    }
  ...
  useEffect(() => {
    if (user) {
      ...
      posthog?.capture('session_started', {
        ...
        user_id: user.id,
        is_new_user: user.created_at ? new Date(user.created_at).getTime() === new Date(user.last_sign_in_at || '').getTime() : false
      });
    }
