...
export default function StoriesScreen() {
	...
	const characterIdsWithStories = useMemo(() => {
		...
		console.log(
			'📖 Sample story characterId:',
			nonArchivedStories[0]?.characterId || (nonArchivedStories[0] as any)?.character_id
		);
		return ids;
	}, [nonArchivedStories]);
	...
	const filteredSections = useMemo(() => {
		...
		const central = stories.filter(
			(story) =>
				story.visibility === 'central' ||
				story.visibility === 'featured' ||
				story.is_central === true
		);
		const user = stories.filter(
			(story) =>
				!story.visibility ||
				story.visibility === 'private' ||
				(story.visibility !== 'central' && story.visibility !== 'featured' && !story.is_central)
	...
	const handleFavoriteToggle = async (storyId: string, isFavorite: boolean) => {
		try {
			...
			Toast.show({
				...
				text1: 'Fehler',
				text2:
					'Favoriten-Status konnte nicht geändert werden. Bei anhaltenden Problemen helfen wir gerne unter support@manacore.ai weiter.',
				position: 'top',
				topOffset: 60,
				visibilityTime: 2000,
			});
		...
		return (
			<View style={styles.cardContainer}>
				<StoryCard
					...
					width={cardWidth}
					showBadge={
						section?.isCentral || item.visibility === 'central' || item.visibility === 'featured'
					}
	...
	const renderSectionHeader = ({ section }) => {
		...
		if (
			section.title === 'Deine Geschichten' &&
	...
	if (loading) {
		return (
			<SafeAreaView style={styles.safeArea} edges={['top']}>
				<CommonHeader title="Geschichten" rightComponent={<SearchButton />} />
