...
export default function CharacterPreview() {
	...
	useEffect(() => {
		...
		if (!shareCode && (!isAuthenticated || !user)) {
			console.error(
				'[CharacterPreview] User not authenticated and no share code provided, redirecting to login'
			);
			setLoading(false);
			Alert.alert(
		...
		try {
			...
			const response = await (shareCode
				? // Public endpoint - no auth required
					fetch(`${process.env.EXPO_PUBLIC_STORYTELLER_BACKEND_URL}${endpoint}`, {
						method: 'GET',
						headers: {
			...
			if (shareCode) {
				setCharacter({
					...
					images_data:
						characterData.imageUrls?.map((url: string) => ({
							image_url: url,
							description: '',
						})) || [],
				...
				{ text: 'Abbrechen', style: 'cancel' },
				{
					...
					onPress: async () => {
						try {
							...
							const response = await fetchWithAuth(`/character/import/${characterId}`, {
								method: 'POST',
							});
							...
							console.log('[CharacterPreview] Character imported successfully:', result.data.id);
	...
	return (
		<SafeAreaView style={styles.container}>
			<CommonHeader title="Charakter importieren" showBackButton />
			...
			<ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
				...
				<View style={styles.infoSection}>
					...
					{character.animal_type && (
						<View style={styles.badge}>
							<Text style={styles.badgeText}>{character.animal_type}</Text>
						</View>
					)}
				</View>
				...
				<View style={styles.actionContainer}>
					...
					{importing && <ActivityIndicator size="small" color="#FF6B9D" style={styles.loader} />}

					<Text style={styles.helpText}>
						Das Importieren dieses Charakters erstellt eine Kopie in deiner Bibliothek, die du für
						Geschichten verwenden kannst.
					</Text>
				</View>
			</ScrollView>
		</SafeAreaView>
