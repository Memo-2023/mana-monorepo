...
export const useParentalGate = () => {
	...
	const openExternalLinkWithGate = async (
		...
		const granted = await requestParentalPermission({
			...
			message:
				options?.message || 'Um diese externe Seite zu öffnen, löse bitte diese Rechenaufgabe:',
		});
	...
	const openEmailWithGate = async (
		...
		const granted = await requestParentalPermission({
			...
			message: options?.message || 'Um eine E-Mail zu senden, löse bitte diese Rechenaufgabe:',
		});
