...
export const dataService = {
  ...
  async getStories(includeArchived: boolean = false): Promise<any[]> {
    try {
      ...
      const transformedStories = (result.data || []).map((story: any) => {
        ...
        if (story.characters_data && Array.isArray(story.characters_data) && story.characters_data.length > 0) {
          // Get the first character's image URL
          characterImageUrl = story.characters_data[0].image_url || story.characters_data[0].imageUrl;
        }
        ...
        return {
          ...
          pages: story.pages_data ? story.pages_data.map((page: any) => ({
  ...
  async getPublicStories(filter?: 'popular' | 'new' | 'featured', page: number = 1, limit: number = 20): Promise<any> {
    try {
  ...
  async publishStory(storyId: string, sharingPreference: 'private' | 'link_only' | 'public'): Promise<any> {
    try {
  ...
  async voteForCharacter(characterId: string, voteType: 'like' | 'love' | 'star'): Promise<boolean> {
    try {
