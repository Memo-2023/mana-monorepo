...
const getApiBaseUrl = () => {
  const envUrl = process.env.EXPO_PUBLIC_STORYTELLER_BACKEND_URL || "https://storyteller-backend-pduya7fsoq-ey.a.run.app";
...
const getDeviceInfo = async () => {
  ...
  const deviceType: 'ios' | 'android' | 'web' = Platform.OS === 'web' 
    ? 'web' 
    : Platform.OS === 'ios' 
      ? 'ios' 
      : 'android';
