...
export class TokenStateInspector {
  ...
  private recordStateChange(newState: TokenState, token?: string, context?: string): void {
    ...
    if (this.lastState !== null && this.lastState !== newState) {
      ...
      console.log(`🔍 TokenStateInspector: State transition: ${this.lastState} → ${newState} (${transition.duration}ms)`);
    }
  ...
  analyzePatterns(): {
    ...
    const refreshTransitions = this.transitions.filter(t => 
      t.from !== TokenState.REFRESHING && t.to === TokenState.REFRESHING
    );
    const successfulRefreshes = this.transitions.filter(t =>
      t.from === TokenState.REFRESHING && t.to === TokenState.VALID
    );
    const failedRefreshes = this.transitions.filter(t =>
      t.from === TokenState.REFRESHING && t.to === TokenState.EXPIRED
    );
    ...
    const averageRefreshTime = refreshTimes.length > 0
    ...
    const refreshSuccessRate = totalRefreshes > 0 
      ? successfulRefreshes.length / totalRefreshes 
      : 0;
  ...
  printReport(): void {
    ...
    console.log(`  Refresh Success Rate: ${(analysis.refreshPatterns.refreshSuccessRate * 100).toFixed(1)}%`);
    console.log(`  Average Refresh Time: ${Math.round(analysis.refreshPatterns.averageRefreshTime)}ms`);
    ...
    Object.entries(analysis.stateDistribution).forEach(([state, count]) => {
      const percentage = this.snapshots.length > 0 
        ? ((count / this.snapshots.length) * 100).toFixed(1)
        : '0';
  ...
  exportData(): {
    ...
    const monitoringDuration = firstSnapshot && lastSnapshot
   ...
   */
  async waitForTransition(fromState: TokenState, toState: TokenState, timeout: number = 5000): Promise<boolean> {
    const startTime = Date.now();
