...
<div class="mx-auto max-w-4xl">
  ...
  {:else if !meal}
    <div class="rounded-2xl bg-white p-8 text-center shadow-lg dark:bg-gray-800">
      ...
      <h2 class="mb-2 text-xl font-semibold text-gray-900 dark:text-white">Mahlzeit nicht gefunden</h2>
        ...
        <div class="rounded-2xl bg-white p-6 shadow-lg dark:bg-gray-800">
          <div class="mb-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              {#if meal.analysis_status === 'pending'}
                <span class="flex items-center gap-1 rounded-full bg-yellow-100 px-2 py-1 text-xs font-medium text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">
            ...
            {#if meal.health_score}
              <div class="flex items-center gap-2">
                ...
                <span class="font-semibold text-gray-900 dark:text-white">{meal.health_score}/10</span>
              </div>
            {/if}
          </div>
          ...
          {#if meal.user_rating && meal.user_rating > 0}
            <div class="mt-3 flex items-center gap-1">
              {#each [1, 2, 3, 4, 5] as star}
                <span class="text-lg {star <= meal.user_rating ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'}">
                  ★
                </span>
              {/each}
