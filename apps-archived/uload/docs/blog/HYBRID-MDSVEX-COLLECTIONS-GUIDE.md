	...
	<div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
		...
		<article class="lg:col-span-3 prose prose-lg max-w-none">
			<header class="not-prose mb-8">
				...
				{#if image}
					<img src={image} alt={title} class="w-full h-64 object-cover rounded-lg mt-6" />
				{/if}
			</header>
		...
		<aside class="lg:col-span-1">
			<div class="sticky top-4 space-y-6">
				...
				<div class="bg-blue-50 p-6 rounded-lg">
					...
					<p class="text-sm text-gray-600 mb-4">Erhalte neue Artikel direkt in dein Postfach.</p>
...
export async function load() {
	...
	const categoryList: BlogCategory[] = Array.from(categories.entries()).map(([name, count]) => ({
		name,
