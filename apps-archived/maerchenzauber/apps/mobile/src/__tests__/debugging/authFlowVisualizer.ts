...
export class AuthFlowVisualizer {
	...
	endFlow(successful?: boolean): void {
		...
		this.currentFlow.summary.retries = this.currentFlow.steps.filter(
			(s) => s.context.retryAttempt && s.context.retryAttempt > 0
		).length;
	...
	findFlows(criteria: {
		...
		return this.flowHistory.filter((flow) => {
			...
			if (
				...
				(!flow.totalDuration || flow.totalDuration < criteria.minDuration)
			) {
				return false;
			}
			...
			if (
				...
				(!flow.totalDuration || flow.totalDuration > criteria.maxDuration)
			) {
				return false;
			}
	...
	analyzeFlowPatterns(): {
		...
		const successRate =
			completedFlows.length > 0 ? (successfulFlows.length / completedFlows.length) * 100 : 0;

		// Average duration
		const durations = completedFlows.filter((f) => f.totalDuration).map((f) => f.totalDuration!);
		...
		const averageRetriesPerFlow =
			this.flowHistory.length > 0 ? totalRetries / this.flowHistory.length : 0;
	...
	printAnalysis(): void {
		...
		console.log(
			`  Average Retries per Flow: ${analysis.retryPatterns.averageRetriesPerFlow.toFixed(1)}`
		);
		...
		Object.entries(analysis.networkImpact.averageDurationByQuality).forEach(
			([quality, duration]) => {
				const failureRate = analysis.networkImpact.failureRateByQuality[quality] || 0;
				console.log(
					`  ${quality}: ${Math.round(duration)}ms avg, ${failureRate.toFixed(1)}% failure rate`
				);
			}
		);
