...
const BASE_BACKEND_URL = process.env.EXPO_PUBLIC_STORYTELLER_BACKEND_URL || 'http://localhost:3002';
...
export const authService = {
		...
		try {
			...
			if (appSupabaseToken) {
				storagePromises.push(
					safeStorage.setItem(STORAGE_KEYS.APP_SUPABASE_TOKEN, appSupabaseToken)
				);
			}
	...
	signUp: async (
		...
		try {
			...
			if (appToken && refreshToken) {
				...
				analytics.track('auth_signup_completed', {
					method: 'email',
	...
	signInWithApple: async (identityToken: string): Promise<{ success: boolean; error?: string }> => {
		try {
			...
			info('Sending Apple Sign-In request to:', `${BACKEND_URL}/auth/apple-signin`);
	...
	signInWithGoogle: async (idToken: string): Promise<{ success: boolean; error?: string }> => {
		try {
			...
			info('Sending Google Sign-In request to:', `${BACKEND_URL}/auth/google-signin`);
	...
	isTokenValidLocally: (token: string): boolean => {
		try {
			...
			if (parts.length !== 3) {
				debug(
					'Token validation failed: not a valid JWT format (expected 3 parts, got ' +
						parts.length +
						')'
				);
				return false;
			...
			if (!isValid || timeUntilExpiry < 60000) {
				// Log if invalid or expires within 1 minute
