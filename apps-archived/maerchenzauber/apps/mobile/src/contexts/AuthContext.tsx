				...
				if (authenticated) {
					...
					if (userData) {
						...
						try {
							...
							console.error(
								'Auth: Error with RevenueCat initialization for existing user:',
								rcError
							);
						...
						console.error(
							'Auth: Failed to retrieve user data despite having valid token - signing out'
						);
	...
	const handleGoogleSignIn = async () => {
		...
		try {
			...
			return {
				success: false,
				error:
					'No user data available after Google Sign-In\n\nIf the problem persists, please contact us at support@manacore.ai',
			};
		} catch (error) {
