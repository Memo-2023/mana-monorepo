	...
	return (
		...
		>
			...
			<Text variant="body" color="#fff" style={styles.modalText}>
				...
				<Text variant="body" color="#FFCB00" style={styles.modalEmailText}>
					{email}
				</Text>{' '}
				gesendet.
			</Text>
...
const ManaModal = ({ visible, onClose }: AppModalProps) => {
	return (
		<Modal visible={visible} onClose={onClose} title="" useBlur={true} maxWidth={360}>
...
const MemoroModal = ({ visible, onClose }: AppModalProps) => {
	return (
		<Modal visible={visible} onClose={onClose} title="" useBlur={true} maxWidth={360}>
			<View style={styles.modalLogoContainer}>
				<Image
					source={require('../assets/images/Memoro-App-Icon.png')}
...
const MaerchenzauberModal = ({ visible, onClose }: AppModalProps) => {
	return (
		<Modal visible={visible} onClose={onClose} title="" useBlur={true} maxWidth={360}>
			...
			<Text variant="body" color="#fff" style={styles.modalTextLeft}>
				Erstelle individuelle Charaktere und lass sie in wundervollen, illustrierten Abenteuern
				lebendig werden.
			</Text>
			<View style={styles.divider} />
	...
	const handleLogin = async () => {
		...
		try {
			...
			if (error instanceof Error) {
				Alert.alert(
					'Anmeldefehler',
					'Fehler bei der Anmeldung. Bitte überprüfe deine E-Mail und dein Passwort.'
				);
			} else {
				Alert.alert(
					'Anmeldefehler',
	...
	return (
		<>
			<ConfirmationModal
				...
				email={email}
			/>
			<ManaModal visible={showManaModal} onClose={() => setShowManaModal(false)} />
			<MemoroModal visible={showMemoroModal} onClose={() => setShowMemoroModal(false)} />
			<MaerchenzauberModal
					...
					>
						<BlurView intensity={60} tint="dark" style={styles.contentContainer}>
									...
									<>
										<View style={styles.successContainer}>
											<Text variant="body" color="#fff" style={styles.successText}>
												...
												<Text variant="body" color="#FFCB00" style={styles.emailHighlight}>
													{resetEmail}
												</Text>{' '}
												gesendet.
											</Text>
									...
									<>
										...
										<View style={styles.inputContainer}>
											<TextInput
												style={[
													styles.input,
													emailError || passwordError ? styles.inputError : null,
												]}
											...
											{mode === 'register-email' && (
												<>
													...
													<Text variant="caption" color="#fff" style={styles.passwordRequirements}>
														Passwort muss mindestens 8 Zeichen lang sein und einen Großbuchstaben,
							...
							<View style={styles.legalContainer}>
								...
								<Pressable
									onPress={() => openExternalLinkWithGate('https://märchen-zauber.de/terms')}
								>
									<Text variant="caption" color="#fff" style={styles.legalLink}>
										AGB
									</Text>
								</Pressable>
								<Text variant="caption" color="#fff" style={styles.legalText}>
								...
								<Pressable
									onPress={() => openExternalLinkWithGate('https://märchen-zauber.de/privacy')}
								>
									<Text variant="caption" color="#fff" style={styles.legalLink}>
