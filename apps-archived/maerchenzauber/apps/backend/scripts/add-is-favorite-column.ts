...
async function runMigration() {
	...
	if (!supabaseUrl || !supabaseServiceKey) {
		...
		console.error(
			'Please set MAERCHENZAUBER_SUPABASE_URL and MAERCHENZAUBER_SUPABASE_SERVICE_ROLE_KEY'
		);
		process.exit(1);
	}
	...
	try {
		...
		const { data: existingColumn, error: checkError } = await supabase.rpc('check_column_exists', {
			table_name: 'stories',
      ...
      `,
		...
		if (error) {
			...
			const { error: directError } = await supabase.from('stories').select('is_favorite').limit(1);
