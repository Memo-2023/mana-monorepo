...
const centralStories = [
	{
		...
		story_prompt:
			'Ein kleiner Drache überwindet seine Höhenangst und lernt mit Hilfe seiner Freunde das Fliegen.',
		story_text:
			'Es war einmal ein kleiner Drache namens Funki, der sich vor dem Fliegen fürchtete...',
		pages_data: [
			{
				page: 1,
	...
	},
	{
		...
		story_text:
			'Als Emma eines Abends in den Himmel schaute, sah sie eine besondere Sternschnuppe...',
		pages_data: [
			{
				page: 1,
...
async function seedCentralStories() {
	...
	try {
		...
		for (let i = 0; i < centralStories.length; i++) {
			...
			const { error: linkError } = await supabase.from('collection_stories').insert({
...
async function createCentralStory(title, prompt, text, pages, metadata = {}) {
	...
	const nextPosition =
		existingStories && existingStories.length > 0 ? existingStories[0].position + 1 : 1;
