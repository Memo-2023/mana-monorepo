...
export class PromptingService {
	...
	constructor(
		...
		if (googleApiKey) {
			...
			this.ai = null;
			this.logger.warn('Gemini not configured - no fallback available for character descriptions');
		}
	}
	...
	public async detectAnimalType(description: string): Promise<Result<string>> {
		try {
			...
			const detectedAnimal = response.data.choices[0].message.content?.trim().toLowerCase() || '';
	...
	public async enhanceAnimalCharacterDescription(
		...
		try {
			if (!this.ai) {
				this.logger.warn('Gemini AI not configured, cannot enhance character description');
				return {
					data: userInput,
					error: null,
				};
...
Return ONLY the enhanced character description, nothing else.`;
	...
	public async createConsistentCharacterDescriptionPrompts(
		...
		if (characterData) {
			...
			this.logger.log(
				`  - Original Description: ${characterData.original_description?.substring(0, 100)}...`
			);
		...
		this.logger.log(`[Story ${storyId}] Attempting character description with Azure OpenAI...`);
		...
		if (this.ai) {
			...
			if (!geminiResult.error && geminiResult.data && geminiResult.data.length > 0) {
				this.logger.log(
			...
			this.logger.error(`[Story ${storyId}] Azure failed and Gemini not available as fallback`);
		}
		...
		try {
			const characterInfo = characterData
				? `The main character is: Name: "${
						...
						characterData.is_animal ? `Animal Type: ${characterData.animal_type}` : ''
					}`
			...
			this.logger.log(`[Story ${storyId}] Azure generated ${characters.length} characters:`);
			characters.forEach((char: any, index: number) => {
				this.logger.log(
					`  Character ${index + 1}: ${char.characterDescription?.substring(0, 100)}...`
				);
			});
			...
			this.logger.error(`[Story ${storyId}] Azure character description error:`, error);
			return {
...
IMPORTANT RULES:
1. The main character's name is "${characterData?.name}" and must be referred to by that name
...
Return a JSON array of character objects with 'characterDescription' (detailed visual description) and 'pages' (array of page numbers).`;
			...
			// Log the generated characters
			this.logger.log(`[Story ${storyId}] Gemini generated ${characters.length} characters:`);
			characters.forEach((char: any, index: number) => {
				this.logger.log(
					`  Character ${index + 1}: ${char.characterDescription?.substring(0, 100)}...`
				);
			...
			this.logger.error(`[Story ${storyId}] Gemini character description error:`, error);
			return {
				error: error instanceof Error ? error.message : String(error),
				data: null,
			};
