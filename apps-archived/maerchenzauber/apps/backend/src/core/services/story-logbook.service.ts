...
export class StoryLogbookService {
	...
	startLogbook(
		...
		const characterIdArray = Array.isArray(characterIds) ? characterIds : [characterIds];
	...
	logCharacter(storyId: string, characterData: any) {
		...
		if (logbook) {
			...
			const existingIndex = logbook.metadata.characters.findIndex((c) => c.id === characterData.id);
			const characterInfo = {
				id: characterData.id,
				name: characterData.name,
				description: characterData.description || characterData.original_description,
	 ...
	 */
	logIllustration(storyId: string, page: number, prompt: string, imageUrl?: string) {
		this.addEntry(storyId, {
	...
	async finalizeLogbook(storyId: string, success = true) {
		...
		logbook.totalDuration =
			new Date(logbook.endTime).getTime() - new Date(logbook.startTime).getTime();
		...
		this.addEntry(storyId, {
			...
			type: 'info',
			data: {
				message: success ? 'Story creation completed successfully' : 'Story creation failed',
	...
	private async saveLogbookToFile(logbook: StoryLogbook) {
		try {
			...
			// Save full JSON log
			await fsPromises.writeFile(filepath, JSON.stringify(logbook, null, 2), 'utf-8');
	...
	private generateSummary(logbook: StoryLogbook): string {
		...
		summary += `- **Success**: ${logbook.metadata.success ? '✅ Yes' : '❌ No'}\n`;
		...
		if (logbook.metadata.characters && logbook.metadata.characters.length > 0) {
			logbook.metadata.characters.forEach((char, index) => {
				...
				summary += `- **ID**: ${char.id}\n`;
				summary += `- **Description**: ${char.description || 'No description'}\n`;
				if (char.isAnimal) {
		...
		summary += `- **Description**: ${logbook.metadata.storyDescription || 'No description'}\n`;
		...
		summary += `- **Illustrator ID**: ${logbook.metadata.illustratorId || 'Unknown'}\n\n`;
		...
		if (!this.isDevelopment) {
			const prompts = logbook.entries.filter((e) => e.type === 'prompt').slice(0, 3);
			if (prompts.length > 0) {
				summary += `## Sample AI Prompts (First ${prompts.length})\n\n`;
				prompts.forEach((prompt, index) => {
					summary += `### Prompt ${index + 1}: ${prompt.step}\n`;
					summary += '```json\n';
					summary += JSON.stringify(prompt.data.prompt, null, 2).substring(0, 500);
					summary += '\n...\n```\n\n';
				});
			}
		} else {
