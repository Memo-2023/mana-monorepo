...
export class SupabaseJsonbAuthService {
	...
	async getUserCharacters(userId: string, token?: string, includeArchived: boolean = false) {
		...
		console.log('getUserCharacters success - found characters:', data?.length || 0);
		return data || [];
	}
	...
	async updateCharacter(characterId: string, updateData: any, token?: string) {
		...
		if (updateData.character_description_prompt !== undefined)
			mappedData.character_description_prompt = updateData.character_description_prompt;
		if (updateData.image_url !== undefined) mappedData.image_url = updateData.image_url;
		if (updateData.source_image_url !== undefined)
			mappedData.source_image_url = updateData.source_image_url;
		if (updateData.is_animal !== undefined) mappedData.is_animal = updateData.is_animal;
		if (updateData.animal_type !== undefined) mappedData.animal_type = updateData.animal_type;
		if (updateData.images_data !== undefined) mappedData.images_data = updateData.images_data;

		// Publishing and sharing fields
		if (updateData.is_published !== undefined) mappedData.is_published = updateData.is_published;
		...
		if (updateData.share_code !== undefined) mappedData.share_code = updateData.share_code;

		// Archiving field
		if (updateData.archived !== undefined) mappedData.archived = updateData.archived;
		...
		if (updateData.characterDescriptionPrompt !== undefined)
			mappedData.character_description_prompt = updateData.characterDescriptionPrompt;
		...
		if (updateData.isAnimal !== undefined) mappedData.is_animal = updateData.isAnimal;
		if (updateData.animalType !== undefined) mappedData.animal_type = updateData.animalType;
		if (updateData.imagesData !== undefined) mappedData.images_data = updateData.imagesData;
		...
		if (error) {
			...
			const errorObj = new Error(error?.message || error?.toString() || 'Database update failed');
	 ...
	 */
	async importCharacter(characterId: string, userId: string, token: string) {
		if (token) {
			this.requestContextService.setToken(token);
		}
		...
		console.log(`importCharacter - user ${userId} importing character ${characterId}`);
		...
		const importedCharacter = await this.createCharacter(userId, newCharacterData, token);
		...
		console.log(`importCharacter success - new character id: ${importedCharacter.id}`);
	...
	async createStory(userId: string, storyData: any, token?: string) {
		...
		console.log('createStory - has token from context:', !!this.requestContextService.getToken());
	...
	async getStoryById(id: string, token?: string) {
		...
		console.log('getStoryById - has token from context:', !!this.requestContextService.getToken());
	...
	async getPublicStories(
		...
		console.log(`getPublicStories - filter: ${filter}, page: ${page}, limit: ${limit}`);
		...
		console.log(`getPublicStories success - found ${data?.length || 0} stories, total: ${count}`);
	...
	async getStoryCollections(token?: string) {
		...
		console.log('getStoryCollections success - found collections:', data?.length || 0);
		return data || [];
	}
	...
	async getCollectionStories(collectionId: string, token?: string) {
		...
		console.log(`getCollectionStories - fetching stories for collection ${collectionId}`);
      ...
      `
		...
		console.log('getCollectionStories success - found stories:', stories.length);
		return stories;
	}
	...
	async voteForStory(
		...
		console.log(`voteForStory - user ${userId} voting for story ${storyId} with type ${voteType}`);
	...
	async unvoteStory(storyId: string, userId: string, token?: string) {
		...
		console.log(`unvoteStory - user ${userId} removing vote from story ${storyId}`);
	...
	async updateStory(storyId: string, updateData: any, token?: string) {
		...
		console.log('updateStory - has token from context:', !!this.requestContextService.getToken());
	...
	async deleteStory(storyId: string, token?: string) {
		...
		console.log('deleteStory - has token from context:', !!this.requestContextService.getToken());
	...
	async getPublicCharacters(filter = 'popular', limit = 20, offset = 0, collectionId?: string) {
		const supabase = this.supabaseProvider.getClient();

		console.log(`getPublicCharacters - filter: ${filter}, limit: ${limit}, offset: ${offset}`);
		...
		console.log(`getPublicCharacters success - found ${data?.length || 0} characters`);
		return data || [];
	}
	...
	async removeVote(characterId: string, userId: string, token?: string) {
		...
		console.log(`removeVote - user ${userId} removing vote from character ${characterId}`);
		...
		if (error) {
			...
			if (error.code === '42P01') {
				console.log('character_votes table does not exist yet, skipping remove vote');
				return { success: true, pending: true };
			}
	...
	async updateCharacterVoteScore(characterId: string) {
		...
		const totalScore = votes?.reduce((sum, vote) => sum + (vote.vote_weight || 1), 0) || 0;
