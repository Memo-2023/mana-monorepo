...
import { CharacterDatabaseOutput, CharacterCreditOutput } from '../types/character-pipeline.types';
...
export const deductCreditsStep: PipelineStep<CharacterDatabaseOutput, CharacterCreditOutput> = {
	name: 'deduct-credits',
	category: 'character',
	...
	execute: async (input, context) => {
		...
		try {
			console.log(`[Credit Deduction] Deducting ${creditsToDeduct} credits for character creation`);
	...
	rollback: async (input, error, context) => {
		if ('transactionId' in input && input.transactionId) {
			console.log(`[Rollback] deduct-credits: Refunding transaction ${input.transactionId}`);
