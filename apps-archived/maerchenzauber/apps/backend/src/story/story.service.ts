...
export class StoryService {
	...
	public async createStoryline(
		...
		this.logger.log(`  - Has character_description: ${!!character.character_description}`);
		this.logger.log(`  - Has original_description: ${!!character.original_description}`);

		const characterDesc = character.character_description || character.original_description;
		if (!characterDesc) {
			this.logger.error(
		...
		try {
			...
			try {
				...
				if (geminiResult.pages) {
					...
					throw new Error('Failed to generate story with both Azure and Gemini');
				}
			}
	...
	public async generateStoryTitle(story: StoryResponse['pages']): AsyncResult<string> {
	...
	public updateStoryPageText(
		...
		const pageIndex = pagesData.findIndex((page) => page.page_number === pageNumber);
