...
export class StoryCreationService {
	...
	async createStory(params: StoryCreationParams): Promise<StoryCreationResult> {
		const { userId, token, characterId, storyDescription, authorId, illustratorId, isAnimalStory } =
			params;
		...
		try {
			...
			const { finalAuthorId, finalIllustratorId } = await this.determineCreators(
				token,
				authorId,
				illustratorId,
				character.is_animal
			...
			if (!isOk(story)) {
				this.logbookService.logError(storyId, 'generate_story', story.error || 'No story data');
				await this.logStoryError(userId, storyId, null, StoryError.CREATE_STORYLINE, isAnimalStory);
			...
			// 6. Generate title
			const title = await this.generateTitle(story.value.pages, userId, storyId, isAnimalStory);
	...
	private async fetchAndValidateCharacter(
		...
		const character = await this.supabaseService.getCharacterById(characterId, token);
		...
		if (character) {
			...
			this.logger.log(
				`  - Original Description: ${character.original_description?.substring(0, 100)}...`
			);
		}
	...
	private async determineCreators(
		...
		const userSettings = await this.settingsClientService.getMaerchenzauberSettings(token);
	...
	private async createCharacterDescriptions(
		...
		const joinedStory = pages.map((page) => `${page.page}: ${page.text}\n`).join(' ');
		...
		this.logger.log(
			`Character original description: ${character.original_description?.substring(0, 100)}...`
		);
		...
		if (createCharacters.error || !createCharacters.data?.length) {
			this.logger.error(`Error creating character descriptions: ${createCharacters.error}`);
		...
		this.logbookService.addEntry(storyId, {
			...
			data: {
				...
				characters: createCharacters.data.map((char: StoryCharacter, idx: number) => ({
					index: idx + 1,
	...
	private async generateIllustrations(
		...
		const illustrationsWithError = illustrationPrompts.filter((ill) => ill.error);
		...
		illustrationPrompts.forEach((prompt) => {
			if (prompt.data) {
				this.logbookService.logIllustration(storyId, prompt.data.page, prompt.data.text);
			}
		});

		// Enhance prompts with extra styling
		const extraPrompts = this.settingsService.getIllustratorPromptWithExtra(illustratorId);
		...
		const characterImageUrl = (character as any).image_url || (character as any).imageUrl;
		...
		if (characterImageUrl) {
			...
			if (signedUrlResult.error) {
				...
				this.logger.log(`Using signed character image URL for consistent generation`);
			}
		}
		...
		if (imagesWithError.length > 0) {
			...
			await this.logImageErrors(userId, storyId, imagesWithError, isAnimalStory);
	...
	private async translateStory(
		...
		const allPagesText = pages.map((page) => `${page.page}. ${page.text}`).join('\n\n');
		const translationResult = await this.coreService.translateToGerman(allPagesText);
		...
		return pages.map((page) => {
			...
			return {
				...
				story_text:
					translationResult.data?.find((p) => p.page === page.page)?.text?.trim() || page.text,
				illustration_description: illustrationPrompts.find((ill) => ill.data.page === page.page)
					?.data.text,
	...
	private async logIllustrationErrors(
		...
		const errorData = {
			error: illustrationsWithError.map((ill) => JSON.stringify(ill.error)).join('\n'),
			storyId,
			error_type: errorType,
		...
		await this.logStoryError(userId, storyId, errorData, errorType, isAnimalStory);
	}
	...
	private async logImageErrors(
		...
		const errors = imagesWithError.map((image) => ({
			error: image.error?.message || image.error?.errors?.message || String(image.error),
