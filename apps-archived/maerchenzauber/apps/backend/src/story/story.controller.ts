...
import { CreateStoryDto, CreateStoryWithAnimalCharacterDto } from './dto/create-story.dto';
...
export class StoryController {
	...
	async getStories(
		...
		this.logger.log(
			`Fetching stories for user ${user.email} (${user.sub}), includeArchived: ${includeArchived}`
		);
	...
	async getStoriesWithCollections(@CurrentUser() user: JwtPayload, @UserToken() token: string) {
		this.logger.log(`Fetching stories with collections for user ${user.email} (${user.sub})`);

		try {
			const result = await this.supabaseService.getStoriesWithCollections(user.sub, token);
			this.logger.log(
	...
	async getPublicStories(
		...
		this.logger.log(`Fetching public stories - filter: ${filter}, page: ${page}, limit: ${limit}`);

		try {
			const pageNum = parseInt(page, 10);
			const limitNum = parseInt(limit, 10);

			const result = await this.supabaseService.getPublicStories(token, filter, pageNum, limitNum);
	...
	async getCollectionStories(@Param('id') collectionId: string, @UserToken() token: string) {
		this.logger.log(`Fetching stories for collection ${collectionId}`);

		try {
			const stories = await this.supabaseService.getCollectionStories(token, collectionId);
	...
	async getStoryById(
		...
		this.logger.log(`Fetching story ${id} for user ${user.email} (${user.sub})`);
	...
	async updateStory(
		...
		this.logger.log(`Updating story ${id} for user ${user.email} (${user.sub})`);
		...
		try {
			...
			const updatedStory = await this.supabaseService.updateStory(id, updateData, token);
	...
	async updateStoryPageText(
		...
		try {
			...
			if (story.user_id !== user.sub) {
				...
				throw new ForbiddenException('You do not have permission to edit this story');
			}
			...
			if (!Array.isArray(currentPagesData) || currentPagesData.length === 0) {
				this.logger.warn(`[StoryController] Story ${storyId} has no pages data`);
			...
			if (!isOk(updateResult)) {
				this.logger.error(`[StoryController] Error updating page: ${updateResult.error.message}`);
			...
			this.logger.log(`[StoryController] Successfully updated page ${pageNum} of story ${storyId}`);
	...
	async voteForStory(
		...
		this.logger.log(
			`User ${user.sub} voting for story ${storyId} with type ${voteData.voteType || 'like'}`
		);
	...
	async unvoteStory(
		...
		try {
			const result = await this.supabaseService.unvoteStory(token, storyId, user.sub);
	...
	async toggleFavorite(
		...
		try {
			...
			return {
				...
				message: body.isFavorite ? 'Story marked as favorite' : 'Story removed from favorites',
			};
		} catch (error) {
	...
	async publishStory(
		...
		try {
			...
			const story = await this.supabaseService.getStoryById(body.storyId, token);
	...
	async deleteStory(
		...
		this.logger.log(`Deleting story ${id} for user ${user.email} (${user.sub})`);
	...
	async createStory(
		...
		try {
			...
			this.logger.log(`Successfully consumed 100 credits for story creation by user ${user.sub}`);
