import { Controller, Put, Body, Param, Logger, Headers, Post } from '@nestjs/common';
...
export class TestController {
	...
	async testUpdate(
		...
		try {
			...
			if (updateError) {
				this.logger.error('Update error - Full object:', JSON.stringify(updateError, null, 2));
				...
				this.logger.error('Error details:', JSON.stringify(updateError.details));
	...
	async testFullUpdate(
		...
		try {
			...
			if (updateData.images_data) mappedData.images_data = updateData.images_data;

			this.logger.log(`Mapped data for update: ${JSON.stringify(mappedData, null, 2)}`);
