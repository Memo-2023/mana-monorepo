<script lang="ts">
	...
	const templates: StoryTemplate[] = [
		...
		},
		{
			...
			prompt:
				'Eine Geschichte in einem magischen Königreich mit Prinzen, Prinzessinnen und Zauberern',
			category: 'fantasy',
		...
		},
		{
			...
			prompt:
				'Eine traumhafte Reise zu den Sternen mit sanften Wolken und friedlichen Himmelsfreunden',
			category: 'bedtime',
	...
	<div class="flex flex-wrap gap-2">
		{#each categories as category}
			<button
				...
				class="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium transition-all {activeCategory ===
				category.id
		...
		<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
			{#each filteredTemplates as template (template.id)}
				...
				>
					<!-- Gradient background -->
					<div
						class="absolute inset-0 bg-gradient-to-br {template.color} opacity-0 transition-opacity group-hover:opacity-10"
					></div>

					<!-- Icon -->
					<div
						class="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br {template.color} shadow-lg"
					>
					...
					<!-- Use button overlay -->
					<div
						class="mt-4 flex items-center gap-2 text-sm font-medium text-pink-600 dark:text-pink-400"
					>
						<span>Verwenden</span>
						<svg
