...
{#if generating}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/90 backdrop-blur">
		<div class="flex flex-col items-center gap-6 p-8 text-center">
			<div class="relative">
				<div
					class="h-24 w-24 animate-spin rounded-full border-4 border-pink-500/30 border-t-pink-500"
				></div>
				<div class="absolute inset-0 flex items-center justify-center">
					<svg
						class="h-10 w-10 animate-pulse text-pink-500"
						fill="none"
						stroke="currentColor"
						viewBox="0 0 24 24"
					>
...
<div class="mx-auto max-w-2xl space-y-8">
	...
	<div class="text-center">
		<h1 class="text-2xl font-bold text-gray-800 dark:text-gray-200 sm:text-3xl">Neuer Charakter</h1>
	...
	{#if generatedImages.length === 0}
		...
		<section class="space-y-4">
			...
			{#if mode === 'photo'}
				<div class="space-y-4">
					<p class="text-sm text-gray-500 dark:text-gray-400">
						Lade ein Foto von einem Kuscheltier hoch, um daraus einen einzigartigen Charakter zu
						erstellen.
					</p>
					...
					{#if uploadedPreview}
						...
						<div class="relative mx-auto w-fit">
							<div
								class="h-48 w-48 overflow-hidden rounded-2xl border-4 border-amber-400 shadow-lg"
							>
								<img
									src={uploadedPreview}
					...
					{:else}
						<!-- Upload Area -->
						<label
							class="flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-amber-400 bg-amber-50/50 p-8 transition-colors hover:bg-amber-50 dark:bg-amber-900/10 dark:hover:bg-amber-900/20"
						>
							<div
			...
			{#if mode === 'description'}
				<div class="space-y-4">
					<p class="text-sm text-gray-500 dark:text-gray-400">
						Beschreibe deinen Charakter mit eigenen Worten. Die KI generiert daraus ein
						einzigartiges Bild.
					</p>
		...
		<section class="space-y-6">
			...
			<div class="flex flex-wrap justify-center gap-4">
				{#each generatedImages as image (image.imageUrl)}
					...
					>
						<div
							class="h-32 w-32 overflow-hidden rounded-full border-3 border-white shadow-lg dark:border-gray-700 sm:h-40 sm:w-40"
						>
							<img
								src={image.imageUrl}
						...
						{#if selectedImage?.imageUrl === image.imageUrl}
							<div
								class="absolute inset-0 flex items-center justify-center rounded-full bg-pink-500/20"
							>
								<div
