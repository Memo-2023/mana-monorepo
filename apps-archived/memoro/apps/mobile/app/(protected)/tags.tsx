...
import { View, ActionSheetIOS, Platform, Alert, FlatList } from 'react-native';
...
export default function TagsScreen() {
	...
	const handleMoveUp = async (tag: Tag) => {
		...
		const newTags = [...tags];
		[newTags[currentIndex - 1], newTags[currentIndex]] = [
			newTags[currentIndex],
			newTags[currentIndex - 1],
		];
	...
	const handleMoveDown = async (tag: Tag) => {
		...
		const newTags = [...tags];
		[newTags[currentIndex], newTags[currentIndex + 1]] = [
			newTags[currentIndex + 1],
			newTags[currentIndex],
		];
