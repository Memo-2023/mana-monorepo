...
export default function CreateBlueprintPage() {
	...
	const [descriptionDE, setDescriptionDE] = useState(
		'Dies ist ein Test-Blueprint mit automatisch ausgefüllten Daten zum Testen der Speicherfunktion.'
	);
	const [descriptionEN, setDescriptionEN] = useState(
		'This is a test blueprint with automatically filled data to test the save function.'
	);
	...
	const [adviceTips, setAdviceTips] = useState<AdviceTip[]>([
		{
			id: 'tip1',
			content: {
				de: 'Erster Tipp: Achte auf die Details',
				en: 'First tip: Pay attention to details',
			},
		},
		{
			id: 'tip2',
			content: { de: 'Zweiter Tipp: Nimm dir Zeit', en: 'Second tip: Take your time' },
		},
		{
			id: 'tip3',
			content: {
				de: 'Dritter Tipp: Mache regelmäßige Pausen',
				en: 'Third tip: Take regular breaks',
			},
		},
		{
			id: 'tip4',
			content: { de: 'Vierter Tipp: Überprüfe deine Arbeit', en: 'Fourth tip: Review your work' },
		},
	]);
	...
	const [prompts, setPrompts] = useState<Prompt[]>([
		{
			...
			prompt_text: {
				de: 'Beschreibe den ersten Schritt im Detail und erkläre, warum er wichtig ist.',
				en: 'Describe the first step in detail and explain why it is important.',
			},
		},
		{
			id: 'prompt2',
			...
			prompt_text: {
				de: 'Analysiere die Ergebnisse und gib Feedback zu den wichtigsten Punkten.',
				en: 'Analyze the results and provide feedback on the most important points.',
			},
		},
	]);
	...
	const addAdviceTip = () => {
		...
		setAdviceTips([...adviceTips, { id: newTipId, content: { de: '', en: '' } }]);
	};
	...
	const validateForm = () => {
		...
		const hasAdviceTip = adviceTips.some(
			(tip) => tip.content.de.trim() !== '' || tip.content.en.trim() !== ''
		);

		if (!hasAdviceTip) {
			setError(
				t('blueprints.error_advice_required', 'Bitte füge mindestens einen Advice-Tipp hinzu.')
			);
			return false;
		}
	...
	const saveBlueprint = async () => {
		...
		try {
			...
			const validAdviceTips = adviceTips.filter(
				(tip) => tip.content.de.trim() !== '' || tip.content.en.trim() !== ''
			);
		...
		>
			<View className="relative flex-1" style={containerStyle}>
				<ScrollView className="flex-1 p-4 pb-24">
					{error && (
						<View className="mb-4 rounded-lg bg-red-100/10 p-3">
					...
					<View className="mb-8 w-full">
						...
						<View className="mb-4">
							<Text className="mb-2 text-sm">{t('blueprints.name_de', 'Name (Deutsch)')}</Text>
							<Input
								value={nameDE}
						...
						<View className="mb-4">
							<Text className="mb-2 text-sm">{t('blueprints.name_en', 'Name (Englisch)')}</Text>
							<Input
								value={nameEN}
						...
						<View className="mb-4">
							<Text className="mb-2 text-sm">
								{t('blueprints.description_de', 'Beschreibung (Deutsch)')}
							</Text>
							<Input
								value={descriptionDE}
								onChangeText={setDescriptionDE}
								placeholder={t(
									'blueprints.description_placeholder_de',
						...
						<View className="mb-4">
							<Text className="mb-2 text-sm">
								{t('blueprints.description_en', 'Beschreibung (Englisch)')}
							</Text>
							<Input
								value={descriptionEN}
								onChangeText={setDescriptionEN}
								placeholder={t(
									'blueprints.description_placeholder_en',
					...
					<View className="mb-8 w-full">
						...
						{adviceTips.map((tip, index) => (
							<View key={tip.id} className="mb-4 rounded-lg border p-3" style={{ borderColor }}>
								...
								<View className="mb-4">
									<Text className="mb-2 text-sm">
										{t('blueprints.tip_content_de', 'Inhalt (Deutsch)')}
									</Text>
									<Input
										value={tip.content.de}
										onChangeText={(text) => updateAdviceTip(tip.id, 'de', text)}
										placeholder={t(
											'blueprints.tip_placeholder_de',
								...
								<View className="mb-4">
									<Text className="mb-2 text-sm">
										{t('blueprints.tip_content_en', 'Inhalt (Englisch)')}
									</Text>
									<Input
					...
					<View className="mb-8 w-full">
						<View className="mb-4 flex-row items-center justify-between">
							<Text className="mb-4 text-lg font-bold">{t('blueprints.prompts', 'Prompts')}</Text>
							<Button
								onPress={addPrompt}
								title={t('common.add', 'Hinzufügen')}
								variant="secondary"
						...
						{prompts.map((prompt, index) => (
							<View key={prompt.id} className="mb-4 rounded-lg border p-3" style={{ borderColor }}>
								<View className="mb-3 flex-row items-center justify-between">
									<Text className="text-base font-bold">
								...
								<View className="mb-4">
									<Text className="mb-2 text-sm">
										{t('blueprints.prompt_title_de', 'Titel (Deutsch)')}
									</Text>
									<Input
										value={prompt.title.de}
										onChangeText={(text) => updatePrompt(prompt.id, 'title', 'de', text)}
										placeholder={t(
											'blueprints.prompt_title_placeholder_de',
											'z.B. Zusammenfassung'
										)}
									/>
								</View>
								...
								<View className="mb-4">
									<Text className="mb-2 text-sm">
										{t('blueprints.prompt_title_en', 'Titel (Englisch)')}
									</Text>
									<Input
								...
								<View className="mb-4">
									<Text className="mb-2 text-sm">
										{t('blueprints.prompt_text_en', 'Prompt-Text (Englisch)')}
									</Text>
									<Input
										value={prompt.prompt_text.en}
										onChangeText={(text) => updatePrompt(prompt.id, 'prompt_text', 'en', text)}
										placeholder={t(
											'blueprints.prompt_text_placeholder_en',
											'z.B. Summarize the following text in detail'
										)}
										textArea
										numberOfLines={3}
									/>
					...
					{/* Speichern-Button */}
					<View className="mb-24 mt-6 w-full items-center" style={{ zIndex: 999 }}>
						{isSubmitting ? (
							<ActivityIndicator size="large" color="#4FC3F7" />
						) : (
							<Pressable
								...
								className="w-full max-w-[200px] rounded-xl bg-blue-500 px-6 py-3 active:bg-blue-700"
								style={{
									elevation: 5,
									shadowColor: '#000',
