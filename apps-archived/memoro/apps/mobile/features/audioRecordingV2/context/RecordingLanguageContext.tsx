...
export const AZURE_SUPPORTED_LANGUAGES = {
	...
	'es-MX': {
		locale: 'es-MX',
		nativeName: LANGUAGES['es-MX'].nativeName + ' ⚡',
		emoji: LANGUAGES['es-MX'].emoji,
	},
	...
	'pt-BR': {
		locale: 'pt-BR',
		nativeName: LANGUAGES['pt-BR'].nativeName + ' ⚡',
		emoji: LANGUAGES['pt-BR'].emoji,
	},
...
export const RecordingLanguageProvider: React.FC<RecordingLanguageProviderProps> = ({
	children,
}) => {
	const [recordingLanguages, setRecordingLanguages] = useState<string[]>([]);
	const [dialectChangeNotification, setDialectChangeNotification] = useState<{
		oldDialect: string;
		newDialect: string;
	} | null>(null);
	...
	const toggleRecordingLanguage = async (language: string) => {
		...
		if (language === 'auto') {
			...
			if (languageFamily) {
				...
				const existingDialect = recordingLanguages.find(
					(lang) => languageFamily.includes(lang) && lang !== language
				);

				if (existingDialect) {
					// Set notification for in-modal display
					const existingName =
						AZURE_SUPPORTED_LANGUAGES[existingDialect]?.nativeName
							.replace(' ⚡', '')
							.replace(' 🐢', '') || existingDialect;
					const newName =
						AZURE_SUPPORTED_LANGUAGES[language]?.nativeName.replace(' ⚡', '').replace(' 🐢', '') ||
						language;
