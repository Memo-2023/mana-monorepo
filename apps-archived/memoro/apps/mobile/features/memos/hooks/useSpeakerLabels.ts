...
export function useSpeakerLabels({
	 ...
	 */
	const handleUpdateSpeakerLabels = useCallback(
		async (speakerMappings: SpeakerMappingInput[]) => {
			if (!memo || !memoId) {
			...
			try {
				...
				Alert.alert(
					'Fehler',
					'Bei der Aktualisierung der Sprecher-Namen ist ein Fehler aufgetreten.'
				);
			} finally {
				if (updateLoadingState) {
