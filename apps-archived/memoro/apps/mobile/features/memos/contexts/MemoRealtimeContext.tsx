...
const MemoRealtimeProviderInner: React.FC<MemoRealtimeProviderProps> = ({ children }) => {
	...
	useEffect(() => {
		...
		if (isAuthenticated && user && !isInitializedRef.current) {
			console.log(
				'MemoRealtimeProvider: Initializing service (user authenticated with valid user object)'
			);
			...
			console.log(
				'MemoRealtimeProvider: Cleaning up service (user not authenticated or no user object)'
			);
	...
	return (
		<MemoRealtimeContext.Provider value={contextValue}>{children}</MemoRealtimeContext.Provider>
	);
};
