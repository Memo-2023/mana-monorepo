...
module.exports = function withAndroidForegroundService(config) {
	return withAndroidManifest(config, async (config) => {
		...
		if (!notifeeService) {
			...
			console.log(
				'✅ Added foreground service declaration with microphone type to Android manifest'
			);
		} else {
			...
			console.log(
				'✅ Updated existing foreground service with microphone type in Android manifest'
			);
		}
