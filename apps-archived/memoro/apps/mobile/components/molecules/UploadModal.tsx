...
const UploadModal: React.FC<UploadModalProps> = ({
	...
	const isInsufficientCreditsModalVisible = useInsufficientCreditsStore(
	...
	const [modalMode, setModalMode] = useState<'upload' | 'language' | 'blueprint' | 'date' | 'time'>(
		'upload'
	);
			...
			if (!result.canceled) {
				...
				try {
					...
					Alert.alert(
						...
						t(
							'upload.duration_extraction_failed',
							'Could not extract media duration. The file may be corrupted or in an unsupported format.'
						),
						[
							{
	...
	const handleUpload = async () => {
		...
		if (!fileDuration) {
			Alert.alert(
				...
				t(
					'upload.no_duration',
					'Media duration could not be determined. Please select a different file.'
				)
			);
			return;
		}
		...
		try {
			console.log(
				'Calling onFileUpload with params:',
				selectedFile,
				selectedLanguageForUpload,
				selectedBlueprintForUpload,
				selectedDateTime,
				fileDuration.milliseconds
			);
			await onFileUpload(
				selectedFile,
				selectedLanguageForUpload,
	...
	const getSelectedLanguageDisplay = () => {
		if (selectedLanguageForUpload === 'auto') {
			...
			return (
				supportedAzureLanguages[selectedLanguageForUpload]?.nativeName || selectedLanguageForUpload
			);
		}
	};
	...
	const getSelectedBlueprintDisplay = () => {
		if (selectedBlueprintForUpload) {
			...
			return (
				selectedBlueprintForUpload.name?.[lang] ||
				selectedBlueprintForUpload.name?.en ||
				selectedBlueprintForUpload.name?.de ||
				t('blueprints.title', 'Blueprints')
			);
		}
					...
					<ScrollView style={{ maxHeight: 400 }} showsVerticalScrollIndicator={true}>
						{Object.entries(supportedAzureLanguages)
							.sort(([a], [b]) => {
								...
								if (!aSelected && bSelected) return 1;
								return supportedAzureLanguages[a].nativeName.localeCompare(
									supportedAzureLanguages[b].nativeName
								);
							})
							.map(([code, language]) => {
								...
								return (
									...
									>
										<Text style={{ fontSize: 20, marginRight: 16 }}>{language.emoji}</Text>
										<View style={{ flex: 1 }}>
											<Text
												style={{
													fontSize: 16,
					...
					<View style={{ width: '100%' }}>
						{isUploading ? (
							...
							<View style={{ alignItems: 'center', paddingVertical: 40 }}>
								...
								>
									{t(
										'upload.processing_info',
										'Die Datei wird verarbeitet und transkribiert. Dies kann einen Moment dauern.'
									)}
								</Text>
							</View>
						) : (
							...
							<>
								...
								>
									{t(
										'upload.description',
										'Select an audio or video file and configure the recording settings. Video files will be automatically converted to audio for transcription.'
									)}
								</Text>
	...
	if (isInsufficientCreditsModalVisible && isVisible) {
		console.log(
			'[UploadModal] Prevented from showing - insufficient credits modal is already visible'
		);
		return null;
	}
					...
					modalMode === 'upload' && !isUploading ? (
						...
						>
							...
							<Button
								title={
									isUploading
										? t('upload.uploading', 'Hochladen...')
										: t('upload.upload', 'Hochladen')
								}
