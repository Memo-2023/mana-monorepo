...
export default function Home() {
    ...
    (payload) => {
      if (payload.event === 'INSERT') {
        ...
        if (shouldReplace) {
          ...
          console.debug('INSERT: Preserved audioDuration from placeholder:', preservedStats.audioDuration);
        }
  ...
  useFocusEffect(
    useCallback(() => {
      // Don't load if we're waiting for a new memo INSERT event
      if (waitingForNewMemoRef.current) {
        console.debug('Skipping loadLatestMemo - waiting for INSERT event:', waitingForNewMemoRef.current);
        return;
      }
          ...
          if (currentMemo) {
            ...
            if (
              ...
              if (!waitingForNewMemoRef.current) {
                ...
                console.debug('Skipping polling loadLatestMemo - waiting for INSERT:', waitingForNewMemoRef.current);
              }
  ...
  const uploadAudioRecording = useCallback(async (
    ...
    try {
      console.debug('Starting audio upload:', { filePath, title, spaceId, blueprintId, durationSeconds, audioFileId });
      ...
      if (uploadResult.success) {
        ...
        if (transcriptionResult.success) {
          ...
          if (audioFileId) {
            ...
            console.debug(`[Upload] Set status to SUCCESS for ${audioFileId} with memoId ${memoId}`);
          }
          ...
          if (audioFileId) {
            ...
            console.debug(`[Upload] Set status to FAILED for ${audioFileId}: Transcription failed`);
          }
        }
      } else {
        ...
        const errorMessage = uploadResult.isNetworkError
          ? t('audio_archive.upload_failed_with_archive_hint', 'Upload failed. Your recording is safely stored in the Audio Archive where you can retry the upload later.')
  ...
  const handleRecordingComplete = useCallback(
    (result: string, title?: string, spaceId?: string | null, blueprintId?: string | null, durationSeconds?: number, audioFileId?: string) => {
      ...
      uploadAudioRecording(result, title || 'Memo', finalSpaceId, finalBlueprintId, finalDuration, audioFileId);
    ...
    [selectedSpaceId, selectedBlueprintId, appendToMemoId, refreshCredits, recordingDuration, track, uploadAudioRecording]
  );
            ...
            <Animated.View style={[styles.memoPreviewContainerAbsolute, { opacity: fadeAnim }]}>
              <MemoPreview
                ...
                hasPhotos={(() => {
                  ...
                  console.debug('INDEX: hasPhotos for memo', latestMemo.id, '=', hasPhotosResult);
