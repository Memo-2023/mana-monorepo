...
import { View, StyleSheet, ActivityIndicator, ScrollView, RefreshControl, Pressable, KeyboardAvoidingView, Platform, Alert } from 'react-native';
...
export default function Prompts() {
  ...
  const handlePromptPress = useCallback((promptId: string) => {
  ...
  const handleUpdatePrompt = useCallback(async (title: string, promptText: string) => {
  ...
  const handleCreatePrompt = useCallback(async (title: string, promptText: string) => {
    try {
      setLoading(true);
  ...
  const filterPrompts = useCallback((filter: string) => {
    ...
    if (newFilter.length === 0) {
      ...
      setFilteredPrompts(prompts.filter(prompt => !prompt.is_public && prompt.user_id === user?.id));
    }
