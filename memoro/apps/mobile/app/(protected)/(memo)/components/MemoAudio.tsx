...
export default function MemoAudio({
  ...
  const handleDeleteAdditionalRecording = useCallback(async (recordingIndex: number) => {
  ...
  if ((!memo?.source?.additional_recordings || memo.source.additional_recordings.length === 0) && !isEditMode) {
    return null;
  }
        ...
        <>
          {memo.source.additional_recordings.map((recording: any, index: number) => (
            <AdditionalRecording
              ...
              memo={memo}
              containerStyle={memo.source?.type === 'combined' ? undefined : { marginHorizontal: 20 }}
            />
          ))}
        </>
      )}
