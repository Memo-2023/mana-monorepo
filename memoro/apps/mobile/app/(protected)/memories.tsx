...
export default function Memories() {
  ...
  const filterMemories = useCallback(() => {
    if (selectedFilterIds.length === 0) {
      ...
      const filtered = memories.filter(memory => 
        selectedFilterIds.includes(memory.title)
      );
  ...
  const handleCopyMemory = useCallback(async (content: string) => {
    try {
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptyText}>
          {t('memories.no_memories', 'Keine Memories gefunden')}
        </Text>
        <Text style={styles.emptySubtext}>
          {t('memories.create_memories_hint', 'Memories werden automatisch erstellt, wenn du Blueprints verwendest.')}
        </Text>
      </View>
    );
  };
