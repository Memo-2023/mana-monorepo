...
export const useOnboardingToasts = () => {
  ...
  const showPageOnboardingToast = useCallback((pageName: PageName) => {
    ...
    if (isLoading || !hasLoadedInitialState || hasSeenPageOnboarding(pageName)) {
      console.debug(`⏭️ Skipping onboarding for ${pageName} - already seen, loading, or not loaded yet`);
      return null;
    }
    ...
    return pageName;
  }, [isLoading, hasLoadedInitialState, hasSeenPageOnboarding, t, pageOnboardingSeen, pageOnboardingModal.visible]);
  ...
  const cleanupPageToast = useCallback((pageName: PageName) => {
