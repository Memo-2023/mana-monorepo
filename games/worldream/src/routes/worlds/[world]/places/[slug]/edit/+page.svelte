<script lang="ts">
	...
	async function loadPlace() {
		try {
			...
			if (!isOwner) {
				throw new Error('Du hast keine Berechtigung, diesen Ort zu bearbeiten');
			}
		} catch (err) {
	...
	async function handleSave(updatedNode: Partial<ContentNode>) {
		...
		goto(`/worlds/${worldSlug}/places/${updated.slug}`);
	}

	function handleCancel() {
		goto(`/worlds/${worldSlug}/places/${slug}`);
	}
...
<svelte:head>
	<title
		>{node?.title ? `${node.title} bearbeiten` : 'Ort bearbeiten'} | {$currentWorld?.title ||
			'Worldream'}</title
	>
</svelte:head>
