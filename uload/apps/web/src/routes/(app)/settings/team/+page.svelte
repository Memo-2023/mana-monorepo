<script lang="ts">
	...
	const remainingSlots = $derived(teamLimit === 0 ? Infinity : teamLimit - (data.teamMembers?.length || 0));
</script>
	...
	<div class="mb-6 rounded-lg bg-theme-surface p-6">
		<div class="flex items-center justify-between">
			<div>
				...
				<p class="text-xl font-semibold text-theme-text capitalize">
		...
		{#if teamLimit > 0 && (data.teamMembers?.length || 0) >= teamLimit}
			<div class="mt-4 rounded-lg bg-theme-warning/10 p-4">
				<div class="flex items-start gap-3">
					<AlertCircle class="h-5 w-5 text-theme-warning" />
					<div>
				...
				{#if form?.success}
					<div class="mt-3">
						...
						<p class="mt-1 text-xs text-yellow-600 dark:text-yellow-400">
							⚠️ E-Mail-Versand ist möglicherweise nicht konfiguriert. Du findest den Einladungslink unten zum manuellen Teilen.
						</p>
					</div>
				{/if}
			{:else}
						...
						<div class="flex items-center justify-between px-6 py-4">
							...
							<div class="flex items-center gap-3">
								{#if member.invitation_status === 'pending'}
									<span class="rounded-full bg-yellow-100 px-3 py-1 text-xs font-medium text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400">
										Pending
									</span>
				...
				<div class="divide-y divide-theme-border">
					...
					{#each data.pendingNewUserInvites || [] as invite}
						<div class="px-6 py-4">
							...
							<div class="mt-2 p-2 bg-gray-50 dark:bg-gray-800 rounded text-xs text-gray-600 dark:text-gray-400 break-all">
								<span class="font-medium">Invite link:</span> {window.location.origin}/register?invite={invite.token}
							</div>
						</div>
					{/each}
				</div>
