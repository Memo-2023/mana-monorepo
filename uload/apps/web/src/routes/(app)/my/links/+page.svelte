<script lang="ts">
	...
	let showFilters = $state(typeof window !== 'undefined' ? localStorage.getItem('showLinksFilters') === 'true' : false);
...
<div class="min-h-screen bg-theme-background">
	<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
			...
			<div class="flex items-center gap-2">
				...
				{#if $viewModes.links !== 'stats'}
					<button
						...
						class="flex items-center gap-2 rounded-lg border border-theme-border px-3 py-2 text-theme-text transition-all hover:bg-theme-surface-hover {isSelectMode ? 'bg-theme-primary text-white hover:bg-theme-primary-hover' : 'bg-theme-surface'}"
				...
				{#if $viewModes.links !== 'stats'}
					<button
						...
						class="flex items-center gap-2 rounded-lg border border-theme-border px-3 py-2 text-theme-text transition-all hover:bg-theme-surface-hover {showFilters ? 'bg-theme-primary text-white hover:bg-theme-primary-hover' : 'bg-theme-surface'}"
				...
				<button
					...
					class="rounded-lg {showCreateForm ? 'bg-theme-surface border border-theme-border' : 'bg-theme-primary'} px-4 py-2 font-medium {showCreateForm ? 'text-theme-text' : 'text-white'} shadow-lg transition-all hover:scale-105 {showCreateForm ? 'hover:bg-theme-surface-hover' : 'hover:bg-theme-primary-hover'}"
				>
		...
		<!-- Filters (collapsible) -->
		{#if showFilters}
			<div class="mb-6 rounded-xl border border-theme-border bg-theme-surface p-6 shadow-xl animate-fade-in">
		...
		{#if isSelectMode && data.links?.items?.length > 0}
			<div class="mb-4 flex items-center gap-4 rounded-lg border border-theme-border bg-theme-surface p-4">
...
{#if showBulkTagModal}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
		<div class="max-w-md w-full rounded-xl bg-theme-surface p-6 shadow-2xl">
			<div class="flex items-center justify-between mb-4">
				<h3 class="text-lg font-semibold text-theme-text">Tags zuweisen</h3>
				<button
					onclick={() => (showBulkTagModal = false)}
					class="p-1 rounded-lg hover:bg-theme-surface-hover transition-colors"
				>
			...
			<div data-bulk-tag-modal class="space-y-4">
				...
				<div class="space-y-2 max-h-60 overflow-y-auto">
					{#each data.tags as tag}
						<label class="flex items-center gap-2 p-2 rounded hover:bg-theme-surface-hover cursor-pointer">
							<input
								type="checkbox"
								value={tag.id}
								class="h-4 w-4 rounded border-theme-border text-theme-primary focus:ring-theme-primary"
		...
		<div class="max-w-md w-full rounded-xl bg-theme-surface p-6 shadow-2xl">
			...
			<div class="flex flex-col items-center gap-4">
				<div class="p-4 bg-white rounded-lg">
					<img
						src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data={encodeURIComponent(window.location.origin + '/' + qrModalLink.short_code)}"
						alt="QR Code"
						class="w-48 h-48"
					/>
				</div>
				...
				<div class="text-center">
					<p class="text-sm text-theme-text-muted mb-2">Scan to visit:</p>
